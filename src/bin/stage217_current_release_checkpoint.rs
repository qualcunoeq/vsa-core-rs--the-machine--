...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(number(&reports["docs/stage216_hle_curriculum_checkpoint.json"], "cases"), 2500);
    assert_eq!(number(&reports["docs/stage216_hle_curriculum_checkpoint.json"], "false_authorizations"), 0);
    assert_eq!(number(&reports["docs/stage216_hle_curriculum_checkpoint.json"], "replay_not_recorded"), 0);
    let manifest = breadth_first_manifest();
    assert!(manifest.validate().is_empty());
    let report = Report {
        schema: "stage217-current-release-checkpoint-v1", parent_file_sha256: hashes,
        ...
        frontend_cases: 2000, production_route_cases: 1200, production_route_authorized: 780,
        hle_cases: 2500, hle_correct_authorized: 2, hle_false_authorizations: 0,
        hle_replay_verified: 2, hle_replay_not_recorded: 0,
        hle_trace_records: std::str::from_utf8(&trace)?.lines().count(), production_mutations: 0,
    };
    assert_eq!((report.manifest_packs, report.validated_packs), (34, 33));
    assert_eq!((report.frontend_cases, report.production_route_cases, report.production_route_authorized), (2000, 1200, 780));
    assert_eq!((report.hle_cases, report.hle_correct_authorized, report.hle_false_authorizations, report.hle_replay_verified, report.hle_replay_not_recorded, report.hle_trace_records, report.production_mutations), (2500, 2, 0, 2, 0, 2500, 0));
    ...
    println!("stage217 manifest={}/{} routes={}/{} hle={}/{} replay={}/{} false_auth=0", report.manifest_packs, report.validated_packs, report.production_route_authorized, report.production_route_cases, report.hle_correct_authorized, report.hle_cases, report.hle_replay_verified, report.hle_replay_not_recorded);
    Ok(())
}
