...
use the_machine::source_complex_pack::{
    complex_linear_bridge::{bridge_complex_to_real_matrix, BridgeStatus as ComplexBridgeStatus,
        ComplexMatrixBridgeRequest},
    evaluate_complex, ComplexOperation, ComplexRequest, ComplexStatus,
};
use the_machine::source_formula_pack::biology_pack::{evaluate_biology, BiologyOperation, BiologyRequest};
use the_machine::source_formula_pack::chemistry_pack::{evaluate_chemistry, ChemistryOperation, ChemistryRequest};
...
impl HasReplayHash for the_machine::source_complex_pack::complex_linear_bridge::ComplexMatrixBridgeResult {
    fn append_replay_tamper(&mut self) { self.replay_hash.push('x'); }
    fn replay_verified(&self) -> bool { self.replay_verified() }
}
...
fn regression_statistics(mode: Expected, index: usize) -> Outcome {
    ...
    let covariance = slope_value.mul(&x_variance).expect("exact covariance product");
    ...
    if mode == Expected::Refused { stats_request.domain = "continuous_statistics".into(); }
    ...
    let mut slope_request = FormulaRequest {
        ...
        inputs: BTreeMap::from([("covariance_sum".into(), covariance), ("x_variance_sum".into(), x_variance)]),
    ...
    if mode == Expected::Refused { slope_request.inputs.insert("x_variance_sum".into(), q(0, 1)); }
    ...
    let intercept = evaluate_regression(&FormulaRequest {
        ...
        inputs: BTreeMap::from([
            ("y_mean".into(), mean),
            ("slope".into(), slope.value.clone().unwrap_or_else(|| q(3, 1))),
            ("x_mean".into(), x_mean),
        ]),
        domain: "source_derived_finite_regression".into(),
    ...
    let exact = match mode {
        ...
        Expected::Ambiguous => stats.status == FormulaStatus::Ambiguous
            && slope.status == FormulaStatus::Ambiguous,
...
fn chemistry_linear(mode: Expected, index: usize) -> Outcome {
    let formula = match index % 3 { 0 => "H2O", 1 => "CO2", _ => "NH3" };
    let chemistry = evaluate_chemistry(&ChemistryRequest {
        operation: ChemistryOperation::ParseFormula,
        formula: Some(formula.into()), reaction: None, from_species: None, to_species: None,
    ...
    let linear = bridge.artifact.as_ref().map(|vector| evaluate_linear_algebra(&LinearAlgebraRequest {
        operation: LinearAlgebraOperation::VectorConstruction,
        matrix: None, vector_a: Some(vector.values.clone()), vector_b: None,
        domain: if mode == Expected::Refused { "probability_vector" } else { "finite_exact_integer" }.into(),
        requested_output: format!("element_count_vector:{}", vector.semantic_kind),
        provenance: bridge.provenance.clone(),
    }));
    let replay = chemistry.replay_verified() && bridge.replay_verified()
        && linear.as_ref().is_none_or(|result| result.replay_verified());
    let tampered = tamper(&chemistry) && tamper(&bridge)
        && linear.as_ref().is_none_or(tamper);
    let authorized = mode == Expected::Supported
        && bridge.status == ChemistryLinearBridgeStatus::Complete
        && linear.as_ref().is_some_and(|result| result.status == LinearAlgebraStatus::Complete)
        && replay;
    let exact = match mode {
        Expected::Supported => authorized,
        Expected::Ambiguous => bridge.status == ChemistryLinearBridgeStatus::Ambiguous,
        Expected::Refused => linear.as_ref().is_none_or(|result| result.status != LinearAlgebraStatus::Complete),
    };
    Outcome { authorized, exact, intermediate_entries: 2 + usize::from(linear.is_some()), replay_verified: replay,
        tamper_rejected: tampered, failure_gate: (!authorized).then(|| "chemistry_linear_boundary".into()) }
}

fn biology_probability(mode: Expected, index: usize) -> Outcome {
    let sequence = match index % 3 { 0 => "AATTGGCC", 1 => "ATCGATCG", _ => "GGCCAATT" };
    let biology = evaluate_biology(&BiologyRequest {
        operation: BiologyOperation::BaseComposition,
        sequence: Some(sequence.into()), orientation: None,
    ...
    let policy = match mode { Expected::Supported => Some("uniform_position"), _ => None };
    let bridge = bridge_base_composition(&biology, policy);
    let probability = bridge.handoff.as_ref().map(|handoff| {
        the_machine::probability_pack::evaluate_probability(&handoff.request)
    });
    let replay = biology.replay_verified() && bridge.replay_verified()
        && probability.as_ref().is_none_or(|result| result.replay_verified());
    let tampered = tamper(&biology) && tamper(&bridge)
        && probability.as_ref().is_none_or(tamper);
    let authorized = mode == Expected::Supported
        && bridge.status == BiologyProbabilityBridgeStatus::Complete
        && probability.as_ref().is_some_and(|result| result.status == the_machine::probability_pack::ProbabilityStatus::Complete)
        && replay;
    let exact = match mode {
        Expected::Supported => authorized,
    ...
    Outcome { authorized, exact, intermediate_entries: 2 + usize::from(probability.is_some()), replay_verified: replay,
        tamper_rejected: tampered, failure_gate: (!authorized).then(|| "biology_probability_policy".into()) }
}
...
fn complex_linear(mode: Expected, index: usize) -> Outcome {
    ...
    let complex = evaluate_complex(&ComplexRequest {
        operation: if mode == Expected::Refused { ComplexOperation::PolarConversion } else { ComplexOperation::Conjugate },
        a: Some(real), b: Some(imag), c: None, d: None,
    ...
    let bridge = bridge_complex_to_real_matrix(&ComplexMatrixBridgeRequest {
        complex: complex_pair, domain: "complex_to_real_matrix_bridge".into(),
        ambiguity: (mode == Expected::Ambiguous).then(|| "matrix representation is unresolved".into()),
        provenance: complex.provenance.clone(),
    });
    let replay = complex.replay_verified() && bridge.replay_verified()
        && bridge.linear_algebra.as_ref().is_none_or(|result| result.replay_verified());
    let tampered = tamper(&complex) && tamper(&bridge)
        && bridge.linear_algebra.as_ref().is_none_or(tamper);
    let authorized = mode == Expected::Supported
    ...
    Outcome { authorized, exact, intermediate_entries: 2 + usize::from(bridge.linear_algebra.is_some()), replay_verified: replay,
        tamper_rejected: tampered, failure_gate: (!authorized).then(|| "complex_linear_boundary".into()) }
}
...
fn metric_topology(mode: Expected, index: usize) -> Outcome {
    ...
    let distances = (0..count)
        ...
        .collect::<Vec<_>>();
    let metric_records = extract_metric_definitions(include_str!("../../docs/sources/topology_without_tears_finite_metric_definition.txt"))
        .expect("metric source extracts");
    let metric = evaluate_metric(&MetricRequest {
        operation: MetricOperation::ValidateMetric,
        metric: "finite_metric_axioms".into(),
        points, distances, center: None, target: None, radius: None,
        ...
        .into(),
        ambiguity: (mode == Expected::Ambiguous).then(|| "induced topology policy is unresolved".into()),
    ...
    let topology = bridge.request.as_ref().map(|request| {
        let records = extract_topology_definitions(include_str!("../../docs/sources/topology_without_tears_finite_definition.txt"))
            .expect("topology source extracts");
        evaluate_topology(request, &records)
    });
    let replay = metric.replay_verified() && bridge.replay_verified()
        && topology.as_ref().is_none_or(|result| result.replay_verified());
    let tampered = tamper(&metric) && tamper(&bridge)
        && topology.as_ref().is_none_or(tamper);
    let authorized = mode == Expected::Supported
    ...
    Outcome { authorized, exact, intermediate_entries: 2 + usize::from(topology.is_some()), replay_verified: replay,
        tamper_rejected: tampered, failure_gate: (!authorized).then(|| "metric_topology_boundary".into()) }
}

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let routes = [
        ("regression_statistics", regression_statistics as fn(Expected, usize) -> Outcome),
    ...
    for (route_index, (route, evaluator)) in routes.into_iter().enumerate() {
        for index in 0..120 {
            ...
            receipts.push(Receipt { id: format!("supported_{route_index}_{index:03}"), route: route.into(), expected: Expected::Supported,
                authorized: outcome.authorized, exact: outcome.exact, intermediate_entries: outcome.intermediate_entries,
                replay_verified: outcome.replay_verified, tamper_rejected: outcome.tamper_rejected,
                failure_gate: outcome.failure_gate, false_authorization: false, false_denial: !outcome.authorized });
        }
        for index in 0..40 {
            let outcome = evaluator(Expected::Ambiguous, index);
            receipts.push(Receipt { id: format!("ambiguous_{route_index}_{index:03}"), route: route.into(), expected: Expected::Ambiguous,
                authorized: outcome.authorized, exact: outcome.exact && !outcome.authorized, intermediate_entries: outcome.intermediate_entries,
                replay_verified: outcome.replay_verified, tamper_rejected: outcome.tamper_rejected,
                failure_gate: outcome.failure_gate, false_authorization: outcome.authorized, false_denial: false });
        }
        for index in 0..40 {
            let outcome = evaluator(Expected::Refused, index);
            receipts.push(Receipt { id: format!("refused_{route_index}_{index:03}"), route: route.into(), expected: Expected::Refused,
                authorized: outcome.authorized, exact: outcome.exact && !outcome.authorized, intermediate_entries: outcome.intermediate_entries,
                replay_verified: outcome.replay_verified, tamper_rejected: outcome.tamper_rejected,
                failure_gate: outcome.failure_gate, false_authorization: outcome.authorized, false_denial: false });
        }
    ...
    let report = Report {
        ...
        source: "independently authored source-derived routes; every route validates typed handoffs",
        ...
        refused: receipts.iter().filter(|r| r.expected == Expected::Refused).count(),
        exact_decisions: receipts.iter().filter(|r| r.exact).count(),
        supported_routes: receipts.iter().filter(|r| r.expected == Expected::Supported && r.authorized).count(),
        ...
        route_counts: receipts.iter().fold(BTreeMap::new(), |mut counts, receipt| {
            *counts.entry(receipt.route.clone()).or_insert(0) += 1; counts
        }),
        hle_questions_read: 0, production_registry_mutations: 0, receipts,
    };
    assert_eq!(report.cases, 1_000);
    assert_eq!((report.supported, report.ambiguous, report.refused), (600, 200, 200));
