...
- Max practical concurrency is limited by current low machine usage threshold before this cluster starts.
- Per-job threads: 80."
...
- Starts only when tracked machine usage leaves at least 30 free cores.
- Per-job threads: 80."
...
out={
    ...
    'workers': len(latest),
    'total_games_latest': sum(r.get('games',0) for r in latest.values()),
    'total_plies_latest': sum(r.get('plies',0) for r in latest.values()),
    'first_interval_agreement': avg(first,'interval_agreement_rate'),
    'middle_interval_agreement': avg(mid,'interval_agreement_rate'),
    'final_interval_agreement': avg(last,'interval_agreement_rate'),
    'final_cumulative_agreement': avg(list(latest.values()),'agreement_rate'),
    'first_interval_loss': avg(first,'interval_avg_loss'),
    'final_interval_loss': avg(last,'interval_avg_loss'),
}
...
log_monitor() {
  ...
  python3 - <<PY >> "$MONITOR"
import json, time
print(json.dumps({'timestamp': time.time(), 'cognition_cores': float('$cognition'), 'tracked_cores': float('$tracked'), 'estimated_free_cores': round(80-float('$tracked'),3), 'event': '$event'}, sort_keys=True))
PY
}
while true; do
...
PY
  ...
  elif ! state_has MEMORY_SCALING_ID; then
    if python3 - <<PY
raise SystemExit(0 if (80 - float('$tracked')) >= 30 else 1)
PY
    then
  ...
  elif ! state_has LOW_CONTENTION_ID; then
  ...
  elif ! state_has CHESS_POSTRUN_DONE; then
    if run_chess_postrun_eval; then
      event="chess_postrun_eval"
    fi
  fi
  log_monitor "$cognition" "$tracked" "$event"
  sleep 300
