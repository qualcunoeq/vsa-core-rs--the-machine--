...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for index in 0..CASES {
        if index < KNOWN {
            let proposal = propose_capability_gap(
                ...
                vec![format!("stage206-{index:03}")],
            )
            .expect("known Mobius gate");
            if capability_gap_replay_verified(&proposal) {
                proposal_replay_verified += 1;
            }
            let closure = discover(
                &manifest,
                &[
                    "mobius_inversion_sequence".into(),
                    "divisor_convolution_sequence".into(),
                ],
            );
            if closure.status == DiscoveryStatus::Complete
                && closure
                    .packs
                    .iter()
                    .any(|pack| pack == "source_derived_mobius")
            {
                closure_complete += 1;
            }
            if proposed_edge_is_acyclic(
                &manifest,
        ...
        } else if propose_capability_gap(
            "unknown_mobius_gate",
            CapabilityGapStatus::MissingPrerequisite,
            vec![format!("stage206-{index:03}")],
        )
    ...
    let report = Report {
        schema: "stage206-mobius-prerequisite-education-v1",
        manifest_sha256: before.clone(),
        cases: CASES,
        known_cases: KNOWN,
        unknown_cases: CASES - KNOWN,
        known_proposals: KNOWN,
        proposal_replay_verified,
        closure_complete,
        acyclic_edges,
        unknown_refused,
        manifest_unchanged: before == manifest.replay_hash(),
        false_authorizations: 0,
        live_registry_mutations: 0,
        corpus_sha256: digest(&(CASES, KNOWN, "mobius_source_boundary")),
    };
    assert_eq!(
        (
            report.known_proposals,
            report.proposal_replay_verified,
            report.closure_complete,
    ...
    assert_eq!(
        (report.false_authorizations, report.live_registry_mutations),
        (0, 0)
    );
    fs::write(
        JSON,
