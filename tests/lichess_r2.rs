...
fn test_lichess_r2() {
    ...
    let total: usize = BufReader::new(file)
        .lines()
        .filter(|l| {
            l.as_ref()
                .map(|s| !s.starts_with("fen,") && !s.trim().is_empty())
                .unwrap_or(false)
        })
        .count();
    ...
    let seeded =
        the_machine::chess_learner::seed_from_lichess_csv(&mut brain, csv_path, train_count)
            .unwrap();
    eprintln!(
        "{} positions → {} clusters in {:.1}s",
        seeded,
