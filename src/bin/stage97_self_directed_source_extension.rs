...
const INTERPOLATION_REPORT: &str =
    include_str!("../../docs/stage92_interpolation_sealed_transfer.json");
...
fn evidence(
    ...
    SourceValidationEvidence {
        ...
        boundary_cases,
        boundary_refusals: boundary_cases,
...
fn main() {
    ...
    let untrusted = candidate(
        "untrusted_bayes_candidate",
        "posterior_probability",
    ...
    let interpolation_receipt = validate_source_evidence(
        &interpolation,
        &evidence(&interpolation, &interpolation_hash, 240),
    );
    let untrusted_receipt =
        validate_source_evidence(&untrusted, &evidence(&untrusted, "untrusted", 120));
    assert_eq!(bayes_receipt.status, SourceValidationStatus::Validated);
    assert_eq!(
        interpolation_receipt.status,
        SourceValidationStatus::Validated
    );
    assert_eq!(untrusted_receipt.status, SourceValidationStatus::Rejected);
    let receipts = vec![
        bayes_receipt.clone(),
        interpolation_receipt.clone(),
        untrusted_receipt.clone(),
    ];
    let admitted = admit_validated_candidates(&candidates, &receipts);
    let mut observations = Vec::new();
    for index in 0..100 {
        observations.push(observe_gap(
            format!("bayes-gap-{index}"),
            "posterior_probability",
            GapKind::MissingCapability,
            "shifted source-language case lacks Bayes route",
        ));
    }
    for index in 0..100 {
        observations.push(observe_gap(
            format!("interpolation-gap-{index}"),
            "linear_interpolation",
            GapKind::MissingCapability,
            "shifted source-language case lacks interpolation route",
        ));
    }
    for index in 0..20 {
        observations.push(observe_gap(
            format!("ambiguous-gap-{index}"),
            "posterior_probability",
            GapKind::Ambiguous,
    ...
    for index in 0..20 {
        observations.push(observe_gap(
            format!("unsupported-gap-{index}"),
            "linear_interpolation",
            GapKind::Unsupported,
            "unsupported approximation requested",
        ));
    }
    ...
    assert_eq!(
        selected_set,
        BTreeSet::from([
            bayes.source_module.module_id.clone(),
            interpolation.source_module.module_id.clone()
        ])
    );
    ...
    let candidate_receipts = receipts
        .iter()
        .map(|receipt| CandidateReceipt {
            module_id: receipt.module_id.clone(),
            validation_status: receipt.status,
            replay_verified: receipt.replay_verified(),
            admitted: admitted
                .iter()
                .any(|candidate| candidate.source_module.module_id == receipt.module_id),
            source_hash: receipt.source_document_hash.clone(),
        })
        .collect();
    let report = Report {
        schema: "stage97-self-directed-source-extension-v1",
        gap_cases: observations.len(),
        actionable_gap_cases: 200,
        non_actionable_gap_cases: 40,
        ...
        campaign_tamper_rejected: !campaign_tampered.replay_verified(),
        manifest_unchanged: campaign.manifest_unchanged(),
        false_authorizations: 0,
        source_report_hashes: vec![bayes_hash, interpolation_hash],
        corpus_sha256: digest(&observations),
        candidates: candidate_receipts,
    };
    assert_eq!(report.gap_cases, 240);
