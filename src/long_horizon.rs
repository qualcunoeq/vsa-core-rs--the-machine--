...
use crate::adversarial::{
    analyze as adversarial_analyze, AdversarialInvestigation, AdversarialOutcome,
};
use crate::epistemic::{
    analyze, EpistemicInvestigation, EvidenceQuery, EvidenceRecord, Hypothesis, HypothesisId,
    Recommendation, SourceFailureMode,
};
    ...
    pub fn replay_verified(&self) -> bool {
        self.replay_hash == receipt_hash(&self.terminal, &self.trace)
            ...
            && self
                .trace
                .iter()
                .all(|step| !step.action_id.is_empty() && !step.evidence_id.is_empty())
    }
}
...
fn current_investigation(
    ...
    EpistemicInvestigation {
        ...
        queries: episode.queries.clone(),
        evidence,
        ground_truth: episode.hidden_truth.clone(),
...
fn choose_action(
    ...
    let clean_evidence = evidence
        .iter()
        .filter(|record| record.failure_mode.is_none())
        .cloned()
        .collect::<Vec<_>>();
    let investigation = current_investigation(episode, clean_evidence);
    let analysis = analyze(&investigation);
    let mut candidates = episode
        .actions
        .iter()
        .filter(|action| {
            !evidence
                .iter()
                .any(|record| record.query_id == action.query_id)
                && !used_groups.contains(&action.correlation_group)
        })
        .filter_map(|action| {
            let assessment = analysis.assessments.iter().find(|assessment| {
                assessment.query_id == action.query_id && assessment.discriminating
            })?;
    ...
    candidates.sort_by(|(a, gain_a), (b, gain_b)| {
        gain_b
            .ratio_cmp(gain_a)
...
fn observation_for(
    ...
    let mut outcome = episode
        .hypotheses
        .iter()
        .find(|hypothesis| Some(&hypothesis.id) == episode.hidden_truth.as_ref())
        .and_then(|hypothesis| hypothesis.predictions.get(&action.query_id))
        .cloned()
    ...
    match episode.scenario {
        EpisodeScenario::DisconfirmingObservation if step == 0 => {
            outcome = "b".into();
            failure_mode = Some(SourceFailureMode::AdversarialFabrication);
            source = "faulty-sensor".into();
            group = "faulty".into();
        }
        EpisodeScenario::CorrelatedTrap if action.correlation_group == "camera" => {
            outcome = "b".into();
            failure_mode = Some(SourceFailureMode::CopiedReport);
            source = "camera-derived".into();
        }
        _ => {}
    }
    EvidenceRecord {
        id: format!("{}-evidence-{step}", episode.id),
        query_id: action.query_id.clone(),
        outcome,
        timestamp: step as u64 + 1,
        valid_until: None,
        source,
        reliability: if failure_mode.is_some() { 100 } else { 30 },
        confidence: 90,
        ancestry: vec![group.clone()],
...
fn terminal_for(
    ...
    let adversarial = AdversarialInvestigation {
        id: episode.id.clone(),
        epistemic: investigation,
        expected_outcome: AdversarialOutcome::InsufficientEvidence,
        expected_failure_count: 0,
        ground_truth: episode.hidden_truth.clone(),
    };
    match adversarial_analyze(&adversarial).outcome {
        AdversarialOutcome::BestKnownHypothesis { hypothesis_id } => {
            Some(TerminalOutcome::Resolved(hypothesis_id))
        }
        AdversarialOutcome::NovelHypothesisNeeded => Some(TerminalOutcome::NovelHypothesisNeeded),
        AdversarialOutcome::MultiplePlausibleHypotheses { .. }
        | AdversarialOutcome::InsufficientEvidence => None,
    }
}
...
pub fn run_episode(episode: &LongHorizonEpisode) -> LongHorizonReceipt {
    ...
    for step in 0..episode.max_steps {
        if let Some(result) = terminal_for(episode, &evidence) {
            terminal = Some(result);
            break;
        }
        let Some(action) = choose_action(episode, &evidence, &used_groups) else {
            break;
        };
        ...
        let plausible = match terminal_for(episode, &evidence) {
            Some(TerminalOutcome::Resolved(id)) => vec![id],
            _ => analyze(&current_investigation(episode, evidence.clone())).plausible_hypotheses,
        };
    ...
    let terminal = terminal
        .or_else(|| terminal_for(episode, &evidence))
        .unwrap_or(TerminalOutcome::JustifiedUnresolved);
...
pub fn evaluate_corpus(cases: &[LongHorizonEpisode]) -> LongHorizonBenchmarkReport {
    let mut report = LongHorizonBenchmarkReport {
        cases: cases.len(),
        ..Default::default()
    };
    for episode in cases {
        ...
        report.unsupported_actions += usize::from(receipt.trace.iter().any(|step| {
            !episode
                .actions
                .iter()
                .any(|action| action.id == step.action_id)
        }));
        let mut groups = BTreeSet::new();
        for step in &receipt.trace {
            if let Some(action) = episode
                .actions
                .iter()
                .find(|action| action.id == step.action_id)
            {
                report.redundant_queries +=
                    usize::from(!groups.insert(action.correlation_group.clone()));
            }
        }
        report.premature_resolution += usize::from(
            matches!(receipt.terminal, TerminalOutcome::Resolved(_))
                && matches!(
                    episode.expected,
                    TerminalOutcome::JustifiedUnresolved | TerminalOutcome::NovelHypothesisNeeded
                ),
        );
        report.failure_to_revise += usize::from(
            matches!(episode.scenario, EpisodeScenario::DisconfirmingObservation)
                && !receipt.trace.iter().any(|step| step.revised),
        );
        report.hypothesis_thrashing += usize::from(receipt.trace.windows(3).any(|steps| {
            steps[0].plausible_hypotheses == steps[2].plausible_hypotheses
                && steps[0].plausible_hypotheses != steps[1].plausible_hypotheses
        }));
        report.budget_waste += usize::from(
            receipt.trace.len() == episode.max_steps
                && matches!(receipt.terminal, TerminalOutcome::Resolved(_)),
        );
        report.false_certainty += usize::from(
            matches!(receipt.terminal, TerminalOutcome::Resolved(_))
                && episode.hidden_truth.as_ref().is_some_and(|truth| {
                    receipt.terminal != TerminalOutcome::Resolved(truth.clone())
                }),
        );
...
fn hypothesis(id: &str, outcome_a: &str, outcome_b: &str) -> Hypothesis {
    Hypothesis {
        ...
        predictions: [
            ("q-fast".into(), outcome_a.into()),
            ("q-slow".into(), outcome_b.into()),
            ("q-cam".into(), outcome_a.into()),
            ("q-cam-copy".into(), outcome_a.into()),
            ("q-independent".into(), outcome_b.into()),
        ]
        .into_iter()
        .collect(),
...
fn actions() -> Vec<EvidenceAction> {
    vec![
        EvidenceAction {
            id: "action-fast".into(),
            query_id: "q-fast".into(),
            source: "fast-sensor".into(),
            correlation_group: "fast".into(),
            cost: 1,
        },
        EvidenceAction {
            id: "action-slow".into(),
            query_id: "q-slow".into(),
            source: "slow-sensor".into(),
            correlation_group: "slow".into(),
            cost: 1,
        },
        EvidenceAction {
            id: "action-camera".into(),
            query_id: "q-cam".into(),
            source: "camera".into(),
            correlation_group: "camera".into(),
            cost: 1,
        },
        EvidenceAction {
            id: "action-copy".into(),
            query_id: "q-cam-copy".into(),
            source: "camera-log".into(),
            correlation_group: "camera".into(),
            cost: 1,
        },
        EvidenceAction {
            id: "action-independent".into(),
            query_id: "q-independent".into(),
            source: "independent".into(),
            correlation_group: "independent".into(),
            cost: 1,
        },
    ]
}
...
pub fn synthetic_corpus() -> Vec<LongHorizonEpisode> {
    ...
    for i in 0..120 {
        cases.push(make_episode(
            format!("long-clear-{i:03}"),
            EpisodeScenario::Clear,
            true,
        ));
    }
    for i in 0..50 {
        cases.push(make_episode(
            format!("long-contradiction-{i:03}"),
            EpisodeScenario::DisconfirmingObservation,
            true,
        ));
    }
    for i in 0..50 {
        cases.push(make_episode(
            format!("long-correlated-{i:03}"),
            EpisodeScenario::CorrelatedTrap,
            true,
        ));
    }
    for i in 0..40 {
        cases.push(make_episode(
            format!("long-missing-{i:03}"),
            EpisodeScenario::MissingHypothesis,
            false,
        ));
    }
    for i in 0..40 {
        cases.push(make_episode(
            format!("long-unresolved-{i:03}"),
            EpisodeScenario::Unresolvable,
            true,
        ));
    }
    cases
...
fn make_episode(id: String, scenario: EpisodeScenario, included: bool) -> LongHorizonEpisode {
    let hypotheses = if included {
        vec![hypothesis("h-a", "a", "a"), hypothesis("h-b", "b", "b")]
    } else {
        vec![hypothesis("h-a", "a", "a"), hypothesis("h-b", "b", "b")]
    };
    let (hidden_truth, expected) = match scenario {
        EpisodeScenario::MissingHypothesis => (
            Some(HypothesisId("h-new".into())),
            TerminalOutcome::NovelHypothesisNeeded,
        ),
        EpisodeScenario::Unresolvable => (
            Some(HypothesisId("h-a".into())),
            TerminalOutcome::JustifiedUnresolved,
        ),
        _ => (
            Some(HypothesisId("h-a".into())),
            TerminalOutcome::Resolved(HypothesisId("h-a".into())),
        ),
    };
    let hypotheses = if matches!(scenario, EpisodeScenario::Unresolvable) {
        vec![
            Hypothesis {
                id: HypothesisId("h-a".into()),
                description: "h-a".into(),
                predictions: [
                    ("q-fast".into(), "same".into()),
                    ("q-slow".into(), "same".into()),
                ]
                .into_iter()
                .collect(),
                causal_paths: BTreeMap::new(),
            ...
            Hypothesis {
                id: HypothesisId("h-b".into()),
                description: "h-b".into(),
                predictions: [
                    ("q-fast".into(), "same".into()),
                    ("q-slow".into(), "same".into()),
                ]
