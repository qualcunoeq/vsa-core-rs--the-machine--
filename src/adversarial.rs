...
use crate::epistemic::{
    EpistemicInvestigation, EvidenceRecord, Hypothesis, HypothesisId, SourceFailureMode,
...
impl AdversarialReplayReceipt {
    pub fn replay_verified(&self) -> bool {
        self.replay_hash
            == receipt_hash(
                &self.outcome,
                &self.plausible_hypotheses,
                &self.unexplained_observations,
                &self.source_failures,
                self.duplicate_observations,
                self.causal_mismatches,
            )
    }
}
...
fn receipt_hash(
    ...
    plausible: &[HypothesisId],
    unexplained: &[String],
    failures: &BTreeMap<String, SourceFailureMode>,
    duplicates: usize,
    causal_mismatches: usize,
) -> String {
    ...
    hasher.update(
        ...
        .expect("adversarial receipt serializes"),
    );
...
fn origin(record: &EvidenceRecord) -> String {
    record
        .correlation_group
        .clone()
        .or_else(|| record.ancestry.first().cloned())
        .unwrap_or_else(|| record.source.clone())
}

fn path_compatible(hypothesis: &Hypothesis, query_id: &str, evidence: &EvidenceRecord) -> bool {
    let Some(expected) = hypothesis.causal_paths.get(query_id) else {
        return evidence.causal_path.is_empty();
    };
    evidence.causal_path == *expected || evidence.causal_path.starts_with(expected)
}

fn reliable_records(
    investigation: &AdversarialInvestigation,
) -> (
    Vec<&EvidenceRecord>,
    BTreeMap<String, SourceFailureMode>,
    usize,
) {
    let as_of = investigation
        .epistemic
        .evidence
        .iter()
    ...
    for record in &investigation.epistemic.evidence {
        if let Some(mode) = &record.failure_mode {
            failures.insert(record.source.clone(), mode.clone());
        }
        if !record.valid_at(as_of)
        ...
        if record.failure_mode.is_none() {
            records.push(record);
        }
    }
...
pub fn analyze(investigation: &AdversarialInvestigation) -> AdversarialAnalysis {
    ...
    if records.is_empty() {
        return AdversarialAnalysis {
            outcome: AdversarialOutcome::InsufficientEvidence,
            plausible_hypotheses: investigation
                .epistemic
                .hypotheses
                .iter()
                .map(|h| h.id.clone())
                .collect(),
            unexplained_observations: Vec::new(),
            source_failures,
            duplicate_observations,
            causal_mismatches: 0,
        };
    }
    let mut plausible: BTreeSet<HypothesisId> = investigation
        .epistemic
        .hypotheses
        .iter()
        .map(|h| h.id.clone())
    ...
    for (query_id, query_records) in by_query {
        ...
        let Some(max_support) = support.values().map(BTreeSet::len).max() else {
            continue;
        };
        let winners: Vec<&String> = support
            .iter()
            .filter(|(_, origins)| origins.len() == max_support)
        ...
        let matching: BTreeSet<HypothesisId> = investigation
            .epistemic
            .hypotheses
            .iter()
            .filter(|hypothesis| {
                plausible.contains(&hypothesis.id)
                    && hypothesis.predictions.get(query_id) == Some(outcome)
                    && query_records.iter().any(|record| {
                        record.outcome == *outcome && path_compatible(hypothesis, query_id, record)
                    })
            })
            .map(|h| h.id.clone())
            .collect();
        let lexical = investigation
            .epistemic
            .hypotheses
            .iter()
            .any(|h| plausible.contains(&h.id) && h.predictions.get(query_id) == Some(outcome));
        if !matching.is_empty() {
            plausible = plausible.intersection(&matching).cloned().collect();
        } else {
            if lexical {
                causal_mismatches += 1;
            }
    ...
    let outcome = if !unexplained.is_empty() {
        AdversarialOutcome::NovelHypothesisNeeded
    } else if plausible_hypotheses.len() == 1 {
        AdversarialOutcome::BestKnownHypothesis {
            hypothesis_id: plausible_hypotheses[0].clone(),
        }
    } else if plausible_hypotheses.len() > 1 {
        AdversarialOutcome::MultiplePlausibleHypotheses {
            hypothesis_ids: plausible_hypotheses.clone(),
        }
    } else {
        AdversarialOutcome::NovelHypothesisNeeded
    };
...
pub fn replay(investigation: &AdversarialInvestigation) -> AdversarialReplayReceipt {
    ...
    let replay_hash = receipt_hash(
        &analysis.outcome,
        &analysis.plausible_hypotheses,
        &analysis.unexplained_observations,
        &analysis.source_failures,
        analysis.duplicate_observations,
        analysis.causal_mismatches,
    );
    AdversarialReplayReceipt {
        investigation_id: investigation.id.clone(),
        outcome: analysis.outcome,
        plausible_hypotheses: analysis.plausible_hypotheses,
        unexplained_observations: analysis.unexplained_observations,
        source_failures: analysis.source_failures,
        duplicate_observations: analysis.duplicate_observations,
        causal_mismatches: analysis.causal_mismatches,
        replay_hash,
    }
}
...
pub fn evaluate_corpus(cases: &[AdversarialInvestigation]) -> AdversarialBenchmarkReport {
    let mut report = AdversarialBenchmarkReport {
        cases: cases.len(),
        ..Default::default()
    };
    for case in cases {
        ...
        if let Some(truth) = &case.ground_truth {
            report.resistance_cases += 1;
            report.misleading_resistance +=
                usize::from(analysis.plausible_hypotheses.contains(truth));
        }
        let expected_correlated = case
            .epistemic
            .evidence
            .iter()
            .fold(BTreeSet::new(), |mut seen, evidence| {
                if let Some(group) = &evidence.correlation_group {
                    if !seen.insert((evidence.query_id.clone(), group.clone())) {
                        return seen;
                    }
                }
                seen
            ...
            < case
                ...
                .evidence
                .iter()
                .filter(|e| e.correlation_group.is_some())
                .count();
        report.correlation_aware +=
            usize::from(expected_correlated == (analysis.duplicate_observations > 0));
        report.source_failures_identified +=
            usize::from(analysis.source_failures.len() == case.expected_failure_count);
        if expected_correlated {
            report.overcount_cases += 1;
            report.overcounting_detected += usize::from(analysis.duplicate_observations > 0);
        }
        let expected_causal_mismatch = case
            .epistemic
            .evidence
            .iter()
            .any(|e| e.causal_path == ["other".to_string(), "path".to_string()]);
        report.causal_compatibility +=
            usize::from(expected_causal_mismatch == (analysis.causal_mismatches > 0));
...
fn hypothesis(id: &str, outcome: &str, path: &[&str]) -> Hypothesis {
    Hypothesis {
        ...
        predictions: [("q".into(), outcome.into())].into_iter().collect(),
        causal_paths: [("q".into(), path.iter().map(|item| (*item).into()).collect())]
            .into_iter()
            .collect(),
    }
}

fn query() -> crate::epistemic::EvidenceQuery {
    crate::epistemic::EvidenceQuery {
        id: "q".into(),
        description: "test causal observation".into(),
        cost: 1,
    }
}
...
fn record(
    ...
    source: &str,
    group: Option<&str>,
    failure: Option<SourceFailureMode>,
    causal_path: &[&str],
    reliability: u8,
) -> EvidenceRecord {
    EvidenceRecord {
        ...
        reliability,
        confidence: 90,
        ancestry: group.map(|item| vec![item.into()]).unwrap_or_default(),
        correlation_group: group.map(str::to_string),
        failure_mode: failure,
        causal_path: causal_path.iter().map(|item| (*item).into()).collect(),
    }
}
...
fn case(
    ...
    let ground_truth = match &expected_outcome {
        AdversarialOutcome::BestKnownHypothesis { hypothesis_id } => Some(hypothesis_id.clone()),
        _ => None,
    };
    let base = EpistemicInvestigation {
        id: id.clone(),
        hypotheses,
        queries: vec![query()],
        evidence,
        ground_truth: ground_truth.clone(),
        expected_recommendation: crate::epistemic::Recommendation::NoDiscriminatingEvidence,
    };
    AdversarialInvestigation {
        id,
        epistemic: base,
...
pub fn synthetic_corpus() -> Vec<AdversarialInvestigation> {
    ...
    for i in 0..40 {
        cases.push(case(
            ...
            AdversarialOutcome::BestKnownHypothesis {
                hypothesis_id: HypothesisId("h-a".into()),
            },
            0,
        ));
    }
        ...
        cases.push(case(
            ...
            vec![
                ...
                record(
                    ...
                    "testimony-a",
                    Some("footage"),
                    Some(SourceFailureMode::CopiedReport),
                    &["event", "sensor"],
                    100,
                ),
                record(
    ...
    for (offset, mode) in [
        (0, SourceFailureMode::ClockDrift),
        (30, SourceFailureMode::IdentityConfusion),
        (60, SourceFailureMode::StaleCache),
    ] {
        for i in 0..30 {
            let id = offset + i;
            let stale = matches!(mode, SourceFailureMode::StaleCache);
            let mut bad = record(
                "bad",
                "wrong",
                "faulty",
            ...
            if stale {
                bad.valid_until = Some(0);
            }
            cases.push(case(
                format!("adv-failure-{id:03}"),
                vec![h_a(), h_b()],
                vec![
                    record(
                        "right",
        ...
        cases.push(case(
            ...
            vec![
                ...
                record(
                    ...
                    "attacker",
                    Some("attacker"),
                    Some(SourceFailureMode::AdversarialFabrication),
                    &["event", "sensor"],
                    100,
                ),
                record(
    ...
    for i in 0..20 {
        cases.push(case(
            ...
            AdversarialOutcome::MultiplePlausibleHypotheses {
                hypothesis_ids: vec![HypothesisId("h-a".into()), HypothesisId("h-b".into())],
            },
            0,
        ));
    }
