...
impl ExternalEnvironment {
    ...
    pub fn submit(&mut self, action: &MachineAction) -> EnvironmentReply {
        ...
        if matches!(self.scenario, EnvironmentScenario::ChangingWorld) && !self.changed {
            ...
            events.push(EnvironmentEvent {
                ...
                description: "world-state changed between observations".into(),
            });
        }
        let (available, outcome, source, group, failure_mode, delay) =
            match (&self.scenario, action.query.as_str()) {
                (EnvironmentScenario::UnknownEntity, _) | (_, "entity:unknown") => (
                    false,
                    "unknown".into(),
                    "none".into(),
                    "none".into(),
                ...
                (EnvironmentScenario::UnavailableQuery, "status:primary") => (
                    false,
                    "unavailable".into(),
                    "primary".into(),
                    "primary".into(),
                ...
                (EnvironmentScenario::DelayedResponse, "status:primary") => (
                    true,
                    "stable".into(),
                    "primary".into(),
                    "primary".into(),
                ...
                (EnvironmentScenario::ChangingWorld, "status:primary") => (
                    true,
                    "stable".into(),
                    "primary".into(),
                    "primary".into(),
                ...
                (EnvironmentScenario::ChangingWorld, "status:tertiary") => (
                    true,
                    "changed".into(),
                    "tertiary".into(),
                    "tertiary".into(),
                ...
                (EnvironmentScenario::ChangingWorld, _) => (
                    true,
                    "changed".into(),
                    "secondary".into(),
                    "secondary".into(),
                ...
                (EnvironmentScenario::DeceptiveSource, "status:primary") => (
                    true,
                    "stable".into(),
                    "deceptive".into(),
                    "deceptive".into(),
                    Some("adversarial_fabrication".into()),
                    0,
                ),
                (EnvironmentScenario::Irresolvable, "status:primary") => (
                    true,
                    "stable".into(),
                    "primary".into(),
                    "primary".into(),
                ...
                (EnvironmentScenario::Irresolvable, "status:secondary") => (
                    true,
                    "changed".into(),
                    "secondary".into(),
                    "secondary".into(),
                ...
                (EnvironmentScenario::Irresolvable, _) => (
                    false,
                    "unavailable".into(),
                    "none".into(),
                    "none".into(),
                ...
                (_, "status:secondary") | (_, "status:tertiary") => (
                    true,
                    "stable".into(),
                    action.query.clone(),
                    action.query.clone(),
    ...
    pub fn hidden_terminal(&self) -> ExpectedTerminal {
        match self.scenario {
            EnvironmentScenario::UnknownEntity | EnvironmentScenario::Irresolvable => {
                ExpectedTerminal::JustifiedUnresolved
            }
            _ => ExpectedTerminal::Resolved(
                if matches!(self.scenario, EnvironmentScenario::ChangingWorld) {
                    "changed"
                } else {
                    "stable"
                }
...
fn receipt_hash(terminal: &ExpectedTerminal, steps: &[ProtocolStep], spent: u16) -> String {
    ...
    hasher.update(
        serde_json::to_vec(&(terminal, steps, spent)).expect("protocol receipt serializes"),
    );
...
pub fn run_protocol_episode(episode: &ProtocolEpisode) -> ProtocolReceipt {
    ...
    for (index, query) in candidate_queries.iter().enumerate() {
        ...
        for observation in &observations {
            if observation.failure_mode.is_none() {
                groups
    ...
    let terminal = if let Some(outcome) = groups
        .values()
        .find(|outcome| groups.values().filter(|value| *value == *outcome).count() >= 2)
    {
        ExpectedTerminal::Resolved(outcome.clone())
    } else {
        ExpectedTerminal::JustifiedUnresolved
    };
...
pub fn evaluate_corpus(cases: &[ProtocolEpisode]) -> IndependentEnvironmentReport {
    let mut report = IndependentEnvironmentReport {
        cases: cases.len(),
        ..Default::default()
    };
    for episode in cases {
        ...
        report.calibrated_abstentions += usize::from(
            matches!(episode.expected, ExpectedTerminal::JustifiedUnresolved)
                == matches!(receipt.terminal, ExpectedTerminal::JustifiedUnresolved),
        );
        let mut resolved_at = None;
        let mut seen_groups = BTreeMap::<String, String>::new();
        for (index, step) in receipt.steps.iter().enumerate() {
            for observation in &step.reply.observations {
            ...
            if resolved_at.is_none()
                && seen_groups.values().any(|outcome| {
                    seen_groups
                        .values()
            ...
            {
                resolved_at = Some(index);
            }
        }
        report.unnecessary_actions +=
            usize::from(resolved_at.is_some_and(|index| receipt.steps.len() > index + 1));
        report.delayed_recovery += usize::from(
            !matches!(episode.scenario, EnvironmentScenario::DelayedResponse)
                || receipt.terminal == episode.expected,
        );
        report.unexpected_event_recovery += usize::from(
            !matches!(episode.scenario, EnvironmentScenario::ChangingWorld)
                || receipt.terminal == episode.expected,
        );
        report.unsupported_actions += usize::from(receipt.steps.iter().any(|step| {
            ![
                "status:primary",
                "status:secondary",
                "status:tertiary",
            ...
            .contains(&step.action.query.as_str())
        }));
...
pub fn synthetic_corpus() -> Vec<ProtocolEpisode> {
    ...
    for i in 0..80 {
        cases.push(ProtocolEpisode {
            id: format!("env-clean-{i:03}"),
            scenario: EnvironmentScenario::Clean,
            expected: ExpectedTerminal::Resolved("stable".into()),
            action_budget: 8,
        });
    }
    for i in 0..50 {
        cases.push(ProtocolEpisode {
            id: format!("env-delayed-{i:03}"),
            scenario: EnvironmentScenario::DelayedResponse,
            expected: ExpectedTerminal::Resolved("stable".into()),
            action_budget: 8,
        });
    }
    for i in 0..40 {
        cases.push(ProtocolEpisode {
            id: format!("env-unavailable-{i:03}"),
            scenario: EnvironmentScenario::UnavailableQuery,
            expected: ExpectedTerminal::Resolved("stable".into()),
            action_budget: 8,
        });
    }
    for i in 0..40 {
        cases.push(ProtocolEpisode {
            id: format!("env-changing-{i:03}"),
            scenario: EnvironmentScenario::ChangingWorld,
            expected: ExpectedTerminal::Resolved("changed".into()),
            action_budget: 8,
        });
    }
    for i in 0..30 {
        cases.push(ProtocolEpisode {
            id: format!("env-deceptive-{i:03}"),
            scenario: EnvironmentScenario::DeceptiveSource,
            expected: ExpectedTerminal::Resolved("stable".into()),
            action_budget: 8,
        });
    }
    for i in 0..30 {
        cases.push(ProtocolEpisode {
            id: format!("env-unknown-{i:03}"),
            scenario: EnvironmentScenario::UnknownEntity,
            expected: ExpectedTerminal::JustifiedUnresolved,
            action_budget: 8,
        });
    }
    for i in 0..30 {
        cases.push(ProtocolEpisode {
            id: format!("env-irresolvable-{i:03}"),
            scenario: EnvironmentScenario::Irresolvable,
            expected: ExpectedTerminal::JustifiedUnresolved,
            action_budget: 8,
        });
    }
