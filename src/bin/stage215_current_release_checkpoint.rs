...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        number(
            &reports["docs/stage214_hle_curriculum_checkpoint.json"],
            "cases"
        ),
        2500
    );
    ...
    let report = Report {
        schema: "stage215-current-release-checkpoint-v1",
        ...
        production_route_exact: 1200,
        production_authorized: 780,
        hle_cases: 2500,
        hle_correct_authorized: 2,
        hle_incorrect_authorized: 0,
        hle_false_authorizations: 0,
        hle_pack_invocations: 0,
        hle_trace_records: std::str::from_utf8(&trace)?.lines().count(),
        hle_trace_sha256: digest_bytes(&trace),
    ...
    assert_eq!(
        ...
        (2000, 2000, 1200, 1200, 780)
    );
    assert_eq!(
        (
            report.hle_cases,
            report.hle_correct_authorized,
            report.hle_incorrect_authorized,
            report.hle_false_authorizations,
            report.hle_pack_invocations,
    ...
    println!(
        "stage215 manifest={}/{} frontend={}/{} routes={}/{} hle={}/{} false_auth=0",
