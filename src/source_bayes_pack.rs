...
use crate::source_formula_pack::{evaluate_formula_records, extract_formula_records, FormulaRecord, FormulaRequest, FormulaResult};

pub const DOMAIN: &str = "source_catalog_bayes_rule";

pub fn records() -> Vec<FormulaRecord> {
    extract_formula_records(include_str!("../docs/sources/openstax_bayes_rule_catalog.txt"))
...
mod tests {
    ...
    fn evaluates_cited_bayes_formula() {
        let request = FormulaRequest {
            ...
            domain: DOMAIN.into(), ambiguity: None, provenance: vec!["test".into()],
        };
        let result = evaluate(&request);
        assert_eq!(result.status, crate::source_formula_pack::FormulaStatus::Complete);
