//! Stage 274: frozen HLE probe for the three-candidate shadow portfolio.
//!
//! Economics, geometry, and health-ratio catalogs are offered to a generic
//! route-blind frontend. A candidate may proceed only when exactly one module
//! yields a complete replayable result. No result is authorized in production.
...
const SHADOW_MANIFEST: &str = "docs/stage271_three_candidate_shadow_manifest.json";
const REPORT_JSON: &str = "docs/stage274_hle_staged_portfolio_probe.json";
const REPORT_MD: &str = "docs/stage274_hle_staged_portfolio_probe.md";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut report = Report {
        schema: "stage274-hle-staged-portfolio-probe-v1",
    ...
    assert_eq!(report.frontend_replays, 7500);
    assert_eq!(report.frontend_tamper_rejections, 7500);
    ...
    fs::write(REPORT_MD, format!("# Stage 274 — HLE staged portfolio probe\n\nThe frozen HLE corpus was offered to the three-candidate shadow portfolio only.\n\n* cases: {}\n* source modules / records: {} / {}\n* frontend replay / tamper: {} / {}\n* unique shadow candidates: {}\n* correct / rejected candidates: {} / {}\n* ambiguous or missing: {}\n* unsupported: {}\n* production authorizations: 0\n* false authorizations: 0\n* live manifest / registry mutations: 0 / 0\n\nThis probe does not alter HLE routing or curriculum state.\n\nReproduce with `cargo run --quiet --bin stage274_hle_staged_portfolio_probe`.\n", report.cases, report.source_modules, report.source_records, report.frontend_replays, report.frontend_tamper_rejections, report.unique_shadow_candidates, report.correct_shadow_candidates, report.incorrect_shadow_candidates_rejected, report.ambiguous_or_missing, report.unsupported))?;
    println!("stage274 cases={} unique_candidates={} correct_candidates={} false_auth=0 manifest_mutated=false", report.cases, report.unique_shadow_candidates, report.correct_shadow_candidates);
    Ok(())
}
