...
fn expected(case: &InputCase) -> &'static str {
    if case.family == "ambiguous" {
        "ambiguous"
    } else if case.expected_complete {
        "authorized"
    } else {
        "unsupported"
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for case in input.corpus {
        ...
        let actual = match decision.status {
            RouteStatus::Authorized => "authorized",
        ...
        receipts.push(Receipt {
            ...
            actual: actual.into(),
            selected: decision.selected.map(|domain| format!("{domain:?}")),
            exact: actual == expected,
            replay: replay_verified(&decision),
            tamper: !replay_verified(&tampered),
            false_authorization: expected != "authorized" && actual == "authorized",
            false_denial: expected == "authorized" && actual != "authorized",
        });
    }
    let report = Report {
        schema: "stage213-production-arithmetic-route-checkpoint-v1",
        input_sha256: digest(&input_bytes),
        cases: receipts.len(),
        exact: receipts.iter().filter(|r| r.exact).count(),
        authorized: receipts.iter().filter(|r| r.actual == "authorized").count(),
        ambiguous: receipts.iter().filter(|r| r.actual == "ambiguous").count(),
        unsupported: receipts
            .iter()
            .filter(|r| r.actual == "unsupported")
            .count(),
        replay: receipts.iter().filter(|r| r.replay).count(),
        tamper: receipts.iter().filter(|r| r.tamper).count(),
    ...
    println!(
        "stage213 exact={}/{} authorized={} ambiguous={} unsupported={} replay={} tamper={}",
        report.exact,
        report.cases,
        report.authorized,
        report.ambiguous,
