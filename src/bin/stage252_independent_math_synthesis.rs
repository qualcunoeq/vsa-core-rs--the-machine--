...
const REPORT_JSON: &str = "docs/stage252_independent_math_synthesis.json";
const REPORT_MD: &str = "docs/stage252_independent_math_synthesis.md";
...
fn probe_algebra_number_theory(case: &Case) -> Probe {
    ...
    let provenance = vec![case.id.clone(), "stage252:algebra-number-theory".into()];
...
fn probe_combinatorics_number_theory(case: &Case) -> Probe {
    ...
    let provenance = vec![
        case.id.clone(),
        "stage252:combinatorics-number-theory".into(),
    ];
...
fn probe_probability_dynamics(case: &Case) -> Probe {
    let ambiguous = case.expected == Expected::Ambiguous;
    let refused = case.expected == Expected::Refused;
    let provenance = vec![case.id.clone(), "stage252:probability-dynamics".into()];
...
fn graph_request(operation: GraphOperation, case: &Case) -> GraphRequest {
    GraphRequest {
        ...
        provenance: vec![case.id.clone(), "stage252:graph-probability-linear".into()],
    }
}
...
fn probe_graph_probability_linear(case: &Case) -> Probe {
    ...
    let initial = evaluate_probability(&ProbabilityRequest {
        ...
        evidence: None,
        ambiguity: None,
        provenance: vec![case.id.clone(), "stage252:initial-distribution".into()],
    });
    ...
    let walk = execute_one_step(
        ...
        true,
        if refused { 2 } else { 1 },
        vec![case.id.clone(), "stage252:random-walk".into()],
    );
...
fn probe_linear_algebra_dynamics(case: &Case) -> Probe {
    ...
    let provenance = vec![case.id.clone(), "stage252:linear-algebra-dynamics".into()];
    ...
    routes
        ...
        .flat_map(|route| {
            (0..200).map(move |index| {
                ...
                Case {
                    id: format!("stage252-{}-{index:03}", route_name(route)),
                    route,
                    expected,
                    index,
                }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let report = Report {
        schema: "stage252-independent-math-synthesis-v1",
