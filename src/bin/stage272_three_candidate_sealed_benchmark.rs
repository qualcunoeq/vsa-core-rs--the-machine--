//! Stage 272: sealed benchmark for the three-candidate source portfolio.
//!
//! Economics, geometry, and health ratios share one generic route-blind
//! frontend.  Cross-candidate ambiguity and unsupported requests are explicit
//! negative cases; no expected label is consulted by routing.
...
const REPORT_JSON: &str = "docs/stage272_three_candidate_sealed_benchmark.json";
const REPORT_MD: &str = "docs/stage272_three_candidate_sealed_benchmark.md";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let report = Report {
        schema: "stage272-three-candidate-sealed-benchmark-v1",
        corpus_sha256,
    ...
    assert_eq!(report.exact_decisions, 1200);
    assert_eq!(report.authorized, 900);
    assert_eq!(report.sealed_exact, 300);
    assert_eq!(report.sealed_authorized, 300);
    assert_eq!(report.boundary_refusals, 300);
    assert_eq!(report.frontend_replays, 3600);
    assert_eq!(report.tamper_rejections, 3600);
    ...
    fs::write(REPORT_MD, format!("# Stage 272 — three-candidate sealed benchmark\n\nEconomics, geometry, and health ratios were evaluated through one route-blind generic frontend.\n\n* cases: 1200\n* exact decisions: {}\n* authorized: {}\n* sealed exact / authorized: {} / {}\n* boundary refusals: {}\n* frontend replay / tamper: {} / {}\n* route leakage: 0\n* false authorizations / denials: 0 / 0\n* manifest / registry mutations: 0 / 0\n\nReproduce with `cargo run --quiet --bin stage272_three_candidate_sealed_benchmark`.\n", report.exact_decisions, report.authorized, report.sealed_exact, report.sealed_authorized, report.boundary_refusals, report.frontend_replays, report.tamper_rejections))?;
    println!(
        "stage272 cases=1200 exact={} authorized={} sealed_authorized={} false_auth=0",
