//! Stage 268: clone-only promotion and rollback for the selected economics pack.
//!
//! Promotion is exercised only in cloned manifests.  Regression scenarios
//! must roll back to the immutable parent while preserving its replay hash.
...
const VALIDATION: &str = "docs/stage267_economics_shadow_validation.json";
const REPORT_JSON: &str = "docs/stage268_economics_promotion_rollback.json";
const REPORT_MD: &str = "docs/stage268_economics_promotion_rollback.md";
...
fn candidate(scenario: Scenario) -> CurriculumPack {
    ...
    CurriculumPack {
        ...
        reusable_artifacts: vec!["bounded_economic_formula".into()],
        ...
        selection_reason: "utility-ranked source candidate".into(),
    }
}
...
fn expected(scenario: Scenario) -> (&'static str, bool) {
    match scenario {
        Scenario::Clean => ("promoted", true),
        Scenario::Regression => ("rolled_back", true),
        _ => ("blocked", true),
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for index in 0..240usize {
        ...
        let parent_preserved = clone.replay_hash() == parent_hash;
        let exact = actual == expected;
        receipts.push(Receipt {
            id: format!("stage268-{index:03}"),
            scenario,
            expected,
            actual,
            exact,
            promotion_replay: proposed.replay_hash() == proposal_hash,
            promotion_tamper_rejected: tamper_rejected,
            regression_detected,
            rollback_applied,
    ...
    let report = Report {
        schema: "stage268-economics-promotion-rollback-v1",
