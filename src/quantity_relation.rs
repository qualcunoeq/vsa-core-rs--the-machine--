...
pub fn formalize(prompt: &str) -> QuantityRelationDecision {
    ...
    if let Some(caps) = unit_rate.captures(text) {
        let (count, total) = (1..=6)
            ...
            .find_map(|pair| {
                caps.get(pair[0])
                    .and_then(|left| caps.get(pair[1]).map(|right| (left, right)))
            })
    ...
    if let Some(caps) = conversion.captures(text) {
        ...
        let kind = if ["hours", "minutes", "days", "weeks"].contains(&large.as_str()) {
            "time"
        } else if ["liters", "milliliters"].contains(&large.as_str()) {
            "volume"
        } else if ["pounds", "ounces"].contains(&large.as_str()) {
            "mass"
        } else {
            "length"
        };
    ...
    let conversion_rephrased =
        Regex::new(r"^express (\d+) ([a-z]+) as ([a-z]+), given (\d+) ([a-z]+) per ([a-z]+)\.$")
            .unwrap();
    ...
    if let Some(caps) = sum.captures(text) {
        ...
        return accepted(
            "sum_difference",
            "[first:quantity,second:quantity]>quantity>target".into(),
            "total",
    ...
    if let Some(caps) = remaining.captures(text) {
        let values = caps
            ...
            .collect::<Vec<_>>();
        let (first, second) = if text.starts_with("a container") {
            (values[0], values[1])
        } else {
            (values[1], values[0])
        };
    ...
    if let Some(caps) = linear.captures(text) {
        ...
        return accepted(
            "linear_quantity",
            "[base:quantity,change:quantity]>quantity>linear_target".into(),
            "linear_target",
...
mod tests {
    ...
    fn accepts_expanded_relation_families() {
        ...
        for prompt in prompts {
            let QuantityRelationDecision::Accepted(artifact) = formalize(prompt) else {
    ...
    fn rejects_out_of_scope_and_ambiguous_relations() {
        assert!(matches!(
            formalize("A price changes by 20% each year. What is the final price?"),
            QuantityRelationDecision::Unsupported
        ));
        assert!(matches!(
            formalize("Convert 5 miles to kilometers using the usual conversion."),
            QuantityRelationDecision::Unsupported
        ));
        assert!(matches!(
            formalize("A vehicle travels quickly and the time is missing."),
