...
#[derive(Debug, Deserialize)]
struct Case { prompt: String, outcome: String }
...
fn main() {
    ...
    for case in &corpus.cases {
        ...
        if case.outcome == "supported" && !is_quantity { route_errors += 1; }
        if case.outcome != "supported" && is_quantity { leakage += 1; }
    }
    let legacy = [
        ("Compute 2 + 3", true),
        ("Either compute 2 + 3 directly, or use another route.", false),
        ("A price changes by 20% each year. What is the final price?", false),
    ];
    let legacy_correct = legacy.iter().filter(|(prompt, supported)| {
