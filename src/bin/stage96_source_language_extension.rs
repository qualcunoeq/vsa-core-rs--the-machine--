...
fn case(index: usize) -> Case {
    ...
    Case {
        id: format!("source_language_extension_{index:04}"),
        route,
        text,
        expected,
    ...
    let (actual, authorized, replay, tamper, provenance) = match case.route {
        Route::Interpolation => {
            ...
            let downstream = frontend
                .request
                .as_ref()
                .map(|r| evaluate_formula_records(r, INTERPOLATION_DOMAIN, interpolation_records));
            let authorized = case.expected == Expected::Supported
                && frontend.status == InterpolationFrontendStatus::Complete
                && downstream
                    .as_ref()
                    .is_some_and(|r| r.status == FormulaStatus::Complete && r.replay_verified());
            ...
            (
                actual,
                authorized,
                interpolation_replay(&frontend)
                    && downstream.as_ref().is_none_or(|r| r.replay_verified()),
                !interpolation_replay(&tampered) && downstream_tamper,
        ...
        Route::Bayes => {
            ...
            let probability = frontend.request.as_ref().and_then(|r| {
                ...
                Some(evaluate_probability(&ProbabilityRequest {
                    ...
                    prior_probability: r.inputs.get("prior").cloned(),
                    likelihood: r.inputs.get("likelihood").cloned(),
                    evidence: r.inputs.get("evidence").cloned(),
                    ambiguity: None,
                    provenance: vec![case.id.clone()],
                }))
            });
            let authorized = case.expected == Expected::Supported
                && frontend.status == BayesFrontendStatus::Complete
                && downstream
                    .as_ref()
                    .is_some_and(|r| r.status == FormulaStatus::Complete && r.replay_verified())
                && probability.as_ref().is_some_and(|r| {
                    matches!(r.artifact, Some(ProbabilityArtifact::Scalar(_)))
                        && r.replay_verified()
                });
            let mut tampered = frontend.clone();
            tampered.replay_hash.push('x');
            let downstream_tamper = downstream.as_ref().is_none_or(|r| {
                ...
                c.replay_hash.push('x');
                !c.replay_verified()
            }) && probability.as_ref().is_none_or(|r| {
                let mut c = r.clone();
                c.replay_hash.push('x');
                !c.replay_verified()
            });
            ...
            (
                actual,
                authorized,
                bayes_replay(&frontend)
                    && downstream.as_ref().is_none_or(|r| r.replay_verified())
                    && probability.as_ref().is_none_or(|r| r.replay_verified()),
                !bayes_replay(&tampered) && downstream_tamper,
                !frontend.provenance.is_empty()
                    && downstream.as_ref().is_none_or(|r| !r.provenance.is_empty())
                    && probability
                        .as_ref()
...
fn metrics(rows: &[Receipt]) -> Metrics {
    Metrics {
        ...
        exact: rows
            .iter()
            .filter(|r| {
                (r.expected == Expected::Supported && r.actual == "supported")
                    || (r.expected == Expected::Ambiguous && r.actual == "ambiguous")
                    || (r.expected == Expected::Unsupported && r.actual == "unsupported")
            })
            .count(),
        authorized: rows.iter().filter(|r| r.authorized).count(),
        ambiguities_preserved: rows
            .iter()
            .filter(|r| r.expected == Expected::Ambiguous && r.actual == "ambiguous")
            .count(),
        unsupported_refused: rows
            .iter()
            .filter(|r| r.expected == Expected::Unsupported && r.actual == "unsupported")
            .count(),
        replay_verified: rows.iter().filter(|r| r.replay_verified).count(),
        tamper_rejections: rows.iter().filter(|r| r.tamper_rejected).count(),
        provenance_preserved: rows.iter().filter(|r| r.provenance_preserved).count(),
...
fn main() {
    ...
    assert_eq!(overall.exact, 800);
    assert_eq!(overall.authorized, 480);
    assert_eq!(overall.ambiguities_preserved, 160);
    assert_eq!(overall.unsupported_refused, 160);
    assert_eq!(overall.replay_verified, 800);
    assert_eq!(overall.tamper_rejections, 800);
    assert_eq!(overall.provenance_preserved, 800);
    assert_eq!(overall.false_authorizations, 0);
    assert_eq!(overall.false_denials, 0);
    let development: Vec<_> = receipts
        .iter()
        .zip(&cases)
        .filter(|(_, c)| c.partition == Partition::Development)
        .map(|(r, _)| r.clone())
        .collect();
    let validation: Vec<_> = receipts
        .iter()
        .zip(&cases)
        .filter(|(_, c)| c.partition == Partition::Validation)
        .map(|(r, _)| r.clone())
        .collect();
    let sealed: Vec<_> = receipts
        .iter()
        .zip(&cases)
        .filter(|(_, c)| c.partition == Partition::Sealed)
        .map(|(r, _)| r.clone())
        .collect();
    let report = Report {
        schema: "stage96-source-language-extension-v1",
