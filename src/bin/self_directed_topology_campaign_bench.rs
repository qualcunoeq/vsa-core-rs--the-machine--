...
fn topology_request(index: usize) -> TopologyRequest {
    ...
    TopologyRequest {
        ...
        points,
        open_sets,
        target_set: if index % 5 == 0 { None } else { Some(vec!["a".into(), "b".into()]) },
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for index in 0..300 {
        observations.push(observe_gap(format!("topology_gap_{index:03}"), "finite_topology", GapKind::MissingCapability, "finite topology artifacts are absent from the current route"));
    }
    for index in 0..120 {
        observations.push(observe_gap(format!("open_set_gap_{index:03}"), "open_set", GapKind::MissingCapability, "open-set classification is required"));
    }
    for index in 0..80 {
        observations.push(observe_gap(format!("matrix_gap_{index:03}"), "matrix_artifact", GapKind::MissingCapability, "matrix artifact is already covered by another pack"));
    }
    let candidates = vec![
        SourceModuleCandidate {
            ...
            provides: vec!["finite_topology".into(), "open_set".into(), "closed_set".into(), "interior".into(), "closure".into()],
    ...
    assert_eq!(selected.module_id, "source_derived_finite_topology_candidate");
    assert_eq!(selected.covered_case_count, 420);
    let plan_replay = selected.replay_verified();
    let mut tampered_plan = selected.clone(); tampered_plan.covered_case_count += 1;
    ...
    let mutations = vec![
        ...
        document.replace("AXIOMS: empty;whole;unions;finite_intersections", "AXIOMS: empty;whole"),
        document.replace("MAX_POINTS: 8", "MAX_POINTS: 0"),
        document.replace("END TOPOLOGY", "BEGIN TOPOLOGY"),
        document.replace("ALIASES: finite topology|topological space", "ALIASES: duplicate|duplicate"),
    ];
    let mutations_rejected = mutations.iter().filter(|mutation| extract_topology_definitions(mutation).is_err()).count();
    ...
    let blocked_shortcuts = plans.iter().filter(|plan| plan.status == the_machine::curriculum_campaign::PlanStatus::Blocked).count();
    let false_authorizations = plans.iter().filter(|plan| plan.status == the_machine::curriculum_campaign::PlanStatus::Proposed && (plan.source_ids.is_empty() || plan.independent_exercise_count == 0)).count();
    let shadow_promotable = candidate_is_promotable(selected, 120) && independent_correct == 120 && mutations_rejected == 6;
    assert_eq!(records.len(), 1); assert_eq!(mutations_rejected, 6); assert_eq!(independent_correct, 120); assert_eq!(independent_replay, 120); assert_eq!(independent_tamper, 120); assert_eq!(blocked_shortcuts, 1); assert_eq!(false_authorizations, 0); assert!(shadow_promotable); assert!(plan_replay); assert!(plan_tamper_rejected); assert!(manifest_unchanged(&before, &manifest));
    let report = Report {
        schema: "stage-g-self-directed-topology-acquisition-v1", observed_cases: observations.len(), gap_clusters: cluster_gaps(&observations).len(), candidate_plans: plans.len(), selected_module: selected.module_id.clone(), selected_coverage: selected.covered_case_count, plan_replay, plan_tamper_rejected, source_records: records.len(), source_mutations: mutations.len(), source_mutations_rejected: mutations_rejected, independent_cases: 120, independent_correct, independent_replay, independent_tamper, shadow_promotable, blocked_shortcuts, false_authorizations, production_authorizations: 0, manifest_unchanged: manifest_unchanged(&before, &manifest), corpus_sha256: hash(&(observations, candidates, plans)),
    };
    let serialized = serde_json::to_string_pretty(&report)?;
    std::fs::write("docs/stage-g-self-directed-topology-acquisition.json", format!("{serialized}\n"))?;
    println!("{serialized}");
    Ok(())
}
