...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for line in std::str::from_utf8(&trace)?.lines() {
        ...
        *first_failure_counts.entry(failure.to_string()).or_insert(0) += 1;
        let has_signal = record
            .get("signals")
            .and_then(Value::as_array)
            .is_some_and(|values| !values.is_empty());
        if !has_signal || failure == "authorized_reference_match" {
            continue;
        }
        signal_cases += 1;
    ...
    let selected_modules = campaign
        .rounds
        .iter()
        .filter(|step| step.module_id.is_some())
        .count();
    let report = Report {
        schema: "stage218-hle-gap-education-audit-v1",
        trace_sha256: digest_bytes(&trace),
        trace_cases,
        curriculum_signal_cases: signal_cases,
        typed_actionable_gaps: 0,
        ambiguous_residuals: observations.len(),
        first_failure_counts,
        campaign_replay_verified: campaign.replay_verified(),
        deterministic_rerun: campaign == rerun,
    ...
    assert_eq!(
        (
            ...
            report.ambiguous_residuals
        ),
        (2500, 705, 705)
    );
    assert_eq!(
        (
            report.typed_actionable_gaps,
            report.selected_modules,
            report.resolved_cases,
            report.remaining_cases
        ),
        (0, 0, 0, 705)
    );
    assert!(
    ...
    println!(
        "stage218 trace={} signals={} ambiguous={} selected=0 resolved=0 replay=true",
        report.trace_cases, report.curriculum_signal_cases, report.ambiguous_residuals
    );
    Ok(())
}
