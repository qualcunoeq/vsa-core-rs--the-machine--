...
fn oracle_invert(values: &[i128]) -> Vec<i128> {
    (1..=values.len())
        .map(|n| {
            divisors(n)
                .map(|d| i128::from(mobius(d)) * values[n / d - 1])
                .sum()
        })
        .collect()
}
...
fn oracle_convolve(left: &[i128], right: &[i128]) -> Vec<i128> {
    (1..=left.len())
        .map(|n| divisors(n).map(|d| left[d - 1] * right[n / d - 1]).sum())
        .collect()
}
...
fn request(case: &Case) -> (MobiusRequest, Option<MobiusArtifact>) {
    let values = if case.expected == Expected::Unsupported {
        ...
        (1..=(4 + case.seed % 8))
            .map(|i| (i as i128).pow(2) - (case.seed % 3) as i128)
            .collect()
    };
    let second = values
        .iter()
        .enumerate()
        .map(|(i, value)| value + i as i128 + 1)
        .collect::<Vec<_>>();
    let expected_artifact = match case.operation {
        MobiusOperation::InvertFiniteSequence => Some(MobiusArtifact::InvertedSequence {
            values: oracle_invert(&values),
            index_origin: 1,
        }),
        MobiusOperation::DivisorConvolution => Some(MobiusArtifact::ConvolutionSequence {
            values: oracle_convolve(&values, &second),
            index_origin: 1,
        }),
    };
    let request = MobiusRequest {
        ...
        ambiguity: (case.expected == Expected::Ambiguous)
            .then(|| "sequence indexing or divisor convention is unresolved".into()),
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for case in &corpus {
        ...
        let authorized =
            actual == Expected::Supported && artifact_correct && result.replay_verified();
    ...
    let report = Report {
        ...
        supported_artifacts: receipts
            .iter()
            .filter(|r| r.expected == Expected::Supported && r.artifact_correct)
            .count(),
