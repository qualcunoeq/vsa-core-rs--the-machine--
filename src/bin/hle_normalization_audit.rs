...
fn classify(text: &str) -> (Mechanism, &'static str) {
    ...
    let notation = ["\\math", "\\frac", "\\begin", "\\operatorname", "\\text{", "\\mathrm", "\\mathbf", "\\gamma", "\\mu", "\\sigma", "_{", "^{", "∈", "≤", "≥"];
    if has_any(&lower, &notation) {
        return (Mechanism::SpecialistNotation, "high");
    }
    if lower.contains('$') || lower.contains("\\(") || lower.contains("\\[") || lower.contains("equation") || lower.contains("formula") {
        return (Mechanism::EmbeddedFormula, "high");
    }
    if lower.matches('?').count() > 1 || has_any(&lower, &["this is a ", "part 1", "part 2", "answer separately", "which of the following statements"]) {
        return (Mechanism::NestedQuestionStructure, "medium");
    }
    if has_any(&lower, &["answer with", "write your answer", "comma separated", "standard notation", "true or false", "output only", "format your answer", "exact number"]) {
        return (Mechanism::AnswerFormatConfusion, "high");
    }
    if has_any(&lower, &["according to", "quoted", "citation", "the passage", "the text says", "the following excerpt", "as described above"]) || lower.matches('"').count() >= 2 {
        return (Mechanism::QuotationOrCitationStructure, "medium");
    }
    if has_any(&lower, &[" this ", " that ", " these ", " those ", " it ", " they ", " respectively", "the latter", "the former", "above", "below"]) {
        return (Mechanism::CrossSentenceBinding, "medium");
    }
    if has_any(&lower, &["let ", "denote ", "suppose ", "where ", "for some ", "such that "]) && has_any(&lower, &[" x ", " y ", " z ", "variable", "unknown"]) {
        return (Mechanism::ImplicitVariables, "medium");
    }
    if has_any(&lower, &["bcr", "tcr", "gcn", "vc dimension", "sharppy", "uci", "fen", "pcr", "snv", "bcs", "stp"]) {
        return (Mechanism::UnresolvedAbbreviation, "medium");
    }
    if has_any(&lower, &["arrhenius", "wasserstein", "k-matrix", "bordism", "homotopy", "polygenic", "rawinsonde", "shamir", "call/cc"]) {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let input = env::args().nth(1).unwrap_or_else(|| "/tmp/hle_knowledge_audit_2147e9e.json".into());
    let output = env::args().nth(2).unwrap_or_else(|| "/tmp/hle_normalization_audit_2147e9e.json".into());
    ...
    for row in audit.records.into_iter().filter(|row| row.gap == "apparent_gap_from_normalization") {
        ...
        *mechanisms.entry(mechanism).or_insert(0) += 1;
        ...
        records.push(MechanismRecord { id: row.id, category: row.category, mechanism, confidence: confidence.into(), question: row.question });
    }
    let report = Report {
...
mod tests {
    ...
    fn mechanisms_are_deterministic() {
        ...
        assert_eq!(classify("write your answer as a comma separated list").0, Mechanism::AnswerFormatConfusion);
        assert_eq!(classify("Alice entered. What did it mean?").0, Mechanism::CrossSentenceBinding);
    }
}
