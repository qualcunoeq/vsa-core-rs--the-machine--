...
fn find_binding(text: &str, labels: &[&str]) -> Option<Rational> {
    for label in labels {
        ...
        let token = text[start + label.len()..]
            ...
            .next()?;
        if let Some(value) = parse_rational(token) {
            return Some(value);
        }
    }
    None
...
pub fn formalize_bayes_text(text: &str, case_id: &str) -> BayesFrontendResult {
    ...
    if lower.contains(" or ")
        || lower.matches("posterior").count() > 1
        || lower.contains("either")
        || lower.contains("two possible")
    {
        return result(
            BayesFrontendStatus::Ambiguous,
            None,
            BTreeMap::new(),
            Vec::new(),
            vec!["Bayes target or interpretation is not unique".into()],
            provenance,
        );
    }
    if [
    ...
    if !(lower.contains("bayes") || lower.contains("posterior") || lower.contains("given")) {
        return result(
            BayesFrontendStatus::Unsupported,
            None,
            BTreeMap::new(),
            Vec::new(),
            vec!["a Bayes posterior request is not explicit".into()],
            provenance,
        );
    }
    ...
    let labels = [
        ...
        (
            "likelihood",
            &["likelihood", "p(b|a)", "p(b | a)"] as &[&str],
        ),
        ("evidence", &["evidence", "p(b)", "p(b) ="] as &[&str]),
    ];
    for (name, aliases) in labels {
        if let Some(value) = find_binding(&lower, aliases) {
            bindings.insert(name.into(), value);
        }
    }
    if bindings.len() != 3 {
        return result(
            BayesFrontendStatus::Missing,
            None,
            bindings,
            Vec::new(),
            vec!["prior, likelihood, and positive evidence must be explicitly bound".into()],
            provenance,
        );
    }
    if bindings.values().any(|value| !value.in_unit_interval()) || !bindings["evidence"].positive()
    {
        return result(
            ...
            bindings,
            Vec::new(),
            vec!["all inputs must be probabilities and evidence must be positive".into()],
            provenance,
        );
    }
    ...
    result(
        BayesFrontendStatus::Complete,
        Some(request),
        bindings,
        vec!["explicit prior, likelihood, and evidence bindings".into()],
        Vec::new(),
        provenance,
    )
}
...
mod tests {
    ...
    fn refuses_missing_or_ambiguous_inputs() {
        let missing = formalize_bayes_text(
            "Find the posterior using prior=1/4 and likelihood=1/2.",
            "missing",
        );
        assert_eq!(missing.status, BayesFrontendStatus::Missing);
        let ambiguous = formalize_bayes_text(
