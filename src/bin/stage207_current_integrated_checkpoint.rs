...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(number(&reports["docs/stage200_algebra_number_theory_composition.json"], "false_authorizations"), 0);
    assert_eq!(number(&reports["docs/stage201_current_cross_domain_synthesis.json"], "false_authorizations"), 0);
    assert_eq!(number(&reports["docs/stage202_mobius_source_pack_bench.json"], "false_authorizations"), 0);
    assert_eq!(number(&reports["docs/stage203_mobius_cross_domain_composition.json"], "false_authorizations"), 0);
    assert_eq!(number(&reports["docs/stage204_mobius_defect_repair.json"], "counterexamples"), 140);
    assert_eq!(number(&reports["docs/stage205_curriculum_memory_after_mobius.json"], "records"), 100000);
    assert_eq!(number(&reports["docs/stage206_mobius_prerequisite_education.json"], "unknown_refused"), 60);
    ...
    let report = Report {
        ...
        manifest_validated_packs: manifest.packs.iter().filter(|pack| pack.status == the_machine::curriculum::CurriculumStatus::ShadowValidated).count(),
    ...
    assert_eq!((report.synthesis_cases, report.exact_decisions), (1720, 2020));
    assert_eq!((report.replay_verified, report.tamper_rejected), (102100, 2860));
    assert_eq!((report.defect_counterexamples, report.sandbox_repairs, report.prerequisite_proposals, report.unknown_gates_refused), (140, 140, 240, 60));
