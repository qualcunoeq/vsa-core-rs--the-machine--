...
pub fn route(prompt: &str) -> MixedRouteDecision {
    match formalize(prompt) {
        QuantityRelationDecision::Accepted(artifact) => MixedRouteDecision::QuantityRelation(artifact),
        ...
        QuantityRelationDecision::Unsupported => match decompose(prompt) {
            DecompositionDecision::Sketch(sketch) => MixedRouteDecision::Existing(DecompositionDecision::Sketch(sketch)),
...
mod tests {
    ...
    fn quantity_route_does_not_steal_existing_numeric_route() {
        assert!(matches!(route("Compute 2 + 3"), MixedRouteDecision::Existing(_)));
        assert!(matches!(route("5 notebooks cost 20 dollars. What is the price per notebook?"), MixedRouteDecision::QuantityRelation(_)));
        assert!(matches!(route("A price changes by 20% each year. What is the final price?"), MixedRouteDecision::Unsupported));
    }
}
