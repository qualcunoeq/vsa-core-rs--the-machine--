...
fn arithmetic_request(
    ...
    ArithmeticFunctionRequest {
        ...
        ambiguity: None,
        provenance: vec!["stage203-mobius-composition".into()],
    }
}
...
fn number_request(value: u64) -> NumberTheoryRequest {
    NumberTheoryRequest {
        operation: NumberTheoryOperation::EulerTotient,
        a: None,
        b: None,
        c: None,
        modulus: Some(value),
...
fn request(case: &Case) -> (MobiusRequest, Vec<bool>, Vec<i128>) {
    ...
    for n in 1..=length {
        let value = match case.route {
            ...
            Route::CombinationDivisor => {
                ...
                input_replays.push(result.replay_verified());
                match result.artifact {
                    Some(CombinatoricsArtifact::Scalar(value)) => value as i128,
                    _ => 0,
                }
            }
            Route::TotientCumulative => {
                ...
                input_replays.push(result.replay_verified());
                match result.artifact {
                    Some(NumberTheoryArtifact::Scalar(value)) => value as i128,
                    _ => 0,
                }
            }
        };
    ...
    let mut request = MobiusRequest {
        operation: MobiusOperation::InvertFiniteSequence,
        values: Some(sequence),
        second_values: None,
        domain: "bounded_source_mobius_inversion".into(),
        indexing_declared: true,
        ambiguity: None,
        provenance: vec![format!("stage203-{}", case.id)],
    };
    if case.expected == Expected::Ambiguous {
        request.ambiguity = Some("divisor indexing or source relation is unresolved".into());
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let corpus = (0..CASES)
        .map(|index| {
            ...
            Case {
                id: format!("stage203-{route:?}-{seed:03}"),
                route,
    ...
    for case in &corpus {
        ...
        let actual = match result.status {
            MobiusStatus::Complete => Expected::Supported,
            MobiusStatus::Ambiguous => Expected::Ambiguous,
            _ => Expected::Unsupported,
        };
        let artifact_correct = case.expected == Expected::Supported
            && result.artifact
        ...
        receipts.push(Receipt {
            ...
            source_provenance: !result.source.source_id.is_empty() && !result.provenance.is_empty(),
            input_pack_replays: input_replays.iter().filter(|value| **value).count(),
            invariant_preserved: artifact_correct,
            false_authorization: case.expected != Expected::Supported && authorized,
            false_denial: case.expected == Expected::Supported && !authorized,
        });
    }
    let report = Report {
        schema: "stage203-mobius-cross-domain-composition-v1",
        parent_stage202_sha256: digest(&fs::read("docs/stage202_mobius_source_pack_bench.json")?),
        ...
        authorized_compositions: receipts
            .iter()
            .filter(|r| {
                r.expected == Expected::Supported && r.artifact_correct && r.replay_verified
            })
            .count(),
