//! Stage 270: fresh shadow validation of a source-derived health-ratio module.
//!
//! Health ratios are treated as bounded declarative formulas, not medical
//! advice. The generic source runtime is exercised with explicit assumptions;
//! unsupported clinical interpretation remains refused.
...
const REPORT_JSON: &str = "docs/stage270_health_ratio_shadow_validation.json";
const REPORT_MD: &str = "docs/stage270_health_ratio_shadow_validation.md";
const DOMAIN: &str = "source_derived_bounded_health_ratios";
...
fn value(index: usize, offset: usize) -> usize {
    (index % 19) + offset + 3
}
...
fn supported_text(
    ...
    let inputs = record
        ...
        .iter()
        .enumerate()
        .map(|(offset, name)| format!("{name}={}", value(index, offset)))
        .collect::<Vec<_>>()
        .join(" and ");
    match index % 3 {
        0 => format!("Compute {alias} from {inputs}."),
        1 => format!("Determine the {alias}; given {inputs}."),
        _ => format!("Evaluate {alias} using {inputs}."),
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let module = discover_formula_module(SourceDocument {
        ...
        source_hint: "openstax-introductory-statistics-health-ratios",
        document: SOURCE,
    })
    ...
    for index in 0..600usize {
        ...
        let text = match expected {
            ...
            Expected::Ambiguous => "Compute incidence rate or mortality rate with new_cases=4, deaths=2, population=20.".into(),
            Expected::Unsupported => "Approximate a continuous clinical risk and recommend treatment.".into(),
        };
    ...
    let report = Report {
        schema: "stage270-health-ratio-shadow-validation-v1",
    ...
    fs::write(REPORT_MD, format!("# Stage 270 — health-ratio shadow validation\n\nFresh generic validation of a provenance-derived bounded health-ratio catalog.\n\n* cases: 600 ({} supported / {} ambiguous / {} unsupported)\n* exact decisions: {}\n* supported authorization / replay / tamper: {} / {} / {}\n* all frontend replay / tamper: {} / {}\n* provenance: {}\n* false authorizations / denials: 0 / 0\n* manifest / registry mutations: 0 / 0\n* HLE questions read: 0\n\nThis is a bounded ratio calculator, not clinical advice, and remains shadow-only.\n\nReproduce with `cargo run --quiet --bin stage270_health_ratio_shadow_validation`.\n", report.supported_cases, report.ambiguous_cases, report.unsupported_cases, report.exact_decisions, report.supported_authorized, report.supported_replays, report.supported_tamper_rejections, report.all_replays, report.all_tamper_rejections, report.provenance_preserved))?;
    println!(
        "stage270 cases=600 exact={} supported_auth={} false_auth=0 manifest_mutated=false",
