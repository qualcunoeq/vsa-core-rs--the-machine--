...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for index in 0..RECORDS {
        ...
        let record = MemoryRecord {
            record_id: format!("stage209-{index:06}"),
            domain: domain.clone(),
            artifact_type: artifact.clone(),
            version: format!("v{}", (index / descriptors.len()) % 4 + 1),
            payload: format!("{domain}|{artifact}|{index}"),
            provenance: vec![
                format!("manifest-sha256:{manifest_sha256}"),
                "stage209-current-manifest-memory".into(),
            ],
    ...
    let stale_refused = (0..125)
        ...
        .count();
    let unknown_refused = (0..125)
        .filter(|_| {
            memory
                .retrieve_exact_version("unknown-domain", "unknown-artifact", "v1")
                .is_empty()
        })
        .count();
    ...
    let tamper_rejected = (0..1_000)
        ...
        .count();
    let report = Report {
        schema: "stage209-current-manifest-memory-after-mobius-frontend-v1",
        manifest_sha256,
        validated_packs,
    ...
    assert_eq!(
        (report.validated_packs, report.descriptors, report.records),
        (33, 123, 100_000)
    );
    assert_eq!(
        (
    ...
    println!(
        "stage209 packs={} descriptors={} records={} replay={} tamper={}",
