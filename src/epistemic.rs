    ...
    pub fn replay_verified(&self) -> bool {
        self.replay_hash == receipt_hash(&self.updates, &self.final_plausible)
            && self
                .updates
                .iter()
                .all(|update| !update.evidence_id.is_empty())
...
fn receipt_hash(updates: &[BeliefUpdate], final_plausible: &[HypothesisId]) -> String {
    ...
    hasher.update(
        serde_json::to_vec(&(updates, final_plausible)).expect("epistemic receipt serializes"),
    );
...
fn usable_evidence(investigation: &EpistemicInvestigation) -> Vec<&EvidenceRecord> {
    ...
    investigation
        .evidence
        .iter()
        .filter(|record| record.valid_at(as_of) && score(record) >= MIN_DECISIVE_EVIDENCE_SCORE)
        .collect()
}
...
fn plausible_after_evidence(investigation: &EpistemicInvestigation) -> BTreeSet<HypothesisId> {
    ...
    for (query_id, records) in by_query {
        let Some(max_score) = records.iter().map(|record| score(record)).max() else {
            continue;
        };
        let winners: BTreeSet<&str> = records
            .iter()
            .filter(|record| score(record) == max_score)
        ...
        let matching: BTreeSet<HypothesisId> = investigation
            ...
            .filter(|hypothesis| {
                hypothesis
                    .predictions
                    .get(query_id)
                    .is_some_and(|prediction| prediction == outcome)
            })
...
pub fn analyze(investigation: &EpistemicInvestigation) -> EpistemicAnalysis {
    let plausible: Vec<HypothesisId> = plausible_after_evidence(investigation)
        .into_iter()
        .collect();
    let predictions = investigation
        .hypotheses
        .iter()
        .filter(|hypothesis| plausible.contains(&hypothesis.id))
        .map(|hypothesis| (hypothesis.id.clone(), hypothesis.predictions.clone()))
        .collect::<BTreeMap<_, _>>();
    let mut shared_predictions = BTreeMap::new();
    let mut assessments = Vec::new();
    let mut unsupported = 0;
    for query in &investigation.queries {
        let outcomes: Vec<&str> = plausible
            .iter()
            .filter_map(|id| predictions.get(id)?.get(&query.id).map(String::as_str))
            .collect();
        if outcomes.len() != plausible.len() {
            unsupported += 1;
            continue;
        }
        let mut partitions = BTreeMap::new();
        for outcome in outcomes {
            *partitions.entry(outcome.to_string()).or_insert(0) += 1;
        }
        if partitions.len() == 1 {
            shared_predictions.insert(
                query.id.clone(),
                partitions.keys().next().expect("one outcome").clone(),
            );
        }
        let total = plausible.len() as u64;
        let sum_squares = partitions
            .values()
            .map(|size| (*size as u64) * (*size as u64))
            .sum::<u64>();
        let gain = InformationGain {
            numerator: total * total - sum_squares,
            denominator: total.max(1),
        };
        assessments.push(QueryAssessment {
            query_id: query.id.clone(),
            information_gain: gain,
            partitions: partitions.clone(),
            discriminating: partitions.len() > 1,
        });
    }
    let discriminating: Vec<&QueryAssessment> = assessments
        .iter()
        .filter(|assessment| assessment.discriminating)
        .collect();
    let recommendation = if discriminating.is_empty() {
        Recommendation::NoDiscriminatingEvidence
    } else {
        let best_gain = discriminating
            .iter()
            .map(|assessment| &assessment.information_gain)
            .max_by(|a, b| a.ratio_cmp(b))
            .expect("discriminating query");
        let best: Vec<String> = discriminating
            .iter()
            .filter(|assessment| {
                assessment.information_gain.ratio_cmp(best_gain) == std::cmp::Ordering::Equal
            })
            .map(|assessment| assessment.query_id.clone())
            .collect();
        if best.len() == 1 {
            Recommendation::Recommend {
                query_id: best[0].clone(),
            }
        } else {
            Recommendation::Ambiguous { query_ids: best }
        }
    };
    EpistemicAnalysis {
...
pub fn replay_beliefs(investigation: &EpistemicInvestigation) -> EpistemicReplayReceipt {
    ...
    for (query_id, records) in by_query {
        ...
        plausible = matching;
        let evidence_id = records
            .iter()
            .find(|record| record.outcome == outcome && score(record) == max_score)
            .expect("winning evidence")
            .id
            .clone();
        updates.push(BeliefUpdate {
            evidence_id,
            retained: plausible.iter().cloned().collect(),
            discarded,
            reason: "highest-confidence compatible evidence".into(),
        });
    }
    let final_plausible = plausible.into_iter().collect::<Vec<_>>();
    let replay_hash = receipt_hash(&updates, &final_plausible);
    EpistemicReplayReceipt {
...
pub fn evaluate_corpus(cases: &[EpistemicInvestigation]) -> EpistemicBenchmarkReport {
    let mut report = EpistemicBenchmarkReport {
        cases: cases.len(),
        ..Default::default()
    };
    for case in cases {
        ...
        report.ranking_correct += usize::from(
            case.ground_truth
                .as_ref()
                .is_none_or(|truth| analysis.plausible_hypotheses.contains(truth)),
        );
        report.prediction_correct += usize::from(
            case.ground_truth
                .as_ref()
                .is_none_or(|truth| analysis.predictions.contains_key(truth)),
        );
        report.recommendation_correct +=
            usize::from(analysis.recommendation == case.expected_recommendation);
        report.ambiguity_preserved += usize::from(
            matches!(
                case.expected_recommendation,
                Recommendation::Ambiguous { .. }
            ) && matches!(analysis.recommendation, Recommendation::Ambiguous { .. }),
        );
        report.unsupported_hypotheses += analysis.unsupported_prediction_count;
        report.belief_updates_correct += usize::from(
            case.ground_truth
                .as_ref()
                .is_none_or(|truth| receipt.final_plausible.contains(truth)),
        );
        report.calibration_correct += usize::from(
            case.ground_truth
                .as_ref()
                .map_or(analysis.plausible_hypotheses.len() > 1, |truth| {
                    analysis.plausible_hypotheses.contains(truth)
                }),
        );
...
fn hypothesis(id: &str, predictions: &[(&str, &str)]) -> Hypothesis {
    Hypothesis {
        ...
        predictions: predictions
            .iter()
            .map(|(query, outcome)| ((*query).into(), (*outcome).into()))
            .collect(),
...
pub fn synthetic_corpus() -> Vec<EpistemicInvestigation> {
    ...
    for index in 0..120 {
        let hypotheses = vec![
            hypothesis("h-a", &[("q-best", "a"), ("q-shared", "same")]),
            hypothesis("h-b", &[("q-best", "b"), ("q-shared", "same")]),
            hypothesis("h-c", &[("q-best", "c"), ("q-shared", "same")]),
        ];
        cases.push(EpistemicInvestigation {
            id: format!("epistemic-clear-{index:03}"),
            hypotheses,
            queries: vec![query("q-best"), query("q-shared")],
            evidence: vec![],
            ground_truth: Some(HypothesisId("h-a".into())),
            expected_recommendation: Recommendation::Recommend {
                query_id: "q-best".into(),
            },
        });
    }
    for index in 0..60 {
        let hypotheses = vec![
            hypothesis("h-a", &[("q-a", "yes"), ("q-b", "left")]),
            hypothesis("h-b", &[("q-a", "no"), ("q-b", "right")]),
            hypothesis("h-c", &[("q-a", "yes"), ("q-b", "right")]),
        ];
        cases.push(EpistemicInvestigation {
            id: format!("epistemic-redundant-{index:03}"),
            hypotheses,
            queries: vec![query("q-a"), query("q-b")],
            evidence: vec![evidence("prior", "q-a", "yes", 1, 40)],
            ground_truth: Some(HypothesisId("h-a".into())),
            expected_recommendation: Recommendation::Recommend {
                query_id: "q-b".into(),
            },
    ...
    for index in 0..50 {
        let hypotheses = vec![
            hypothesis("h-a", &[("q-main", "red"), ("q-check", "near")]),
            hypothesis("h-b", &[("q-main", "blue"), ("q-check", "far")]),
            hypothesis("h-c", &[("q-main", "green"), ("q-check", "far")]),
        ];
        cases.push(EpistemicInvestigation {
            id: format!("epistemic-misleading-{index:03}"),
            hypotheses,
            queries: vec![query("q-main"), query("q-check")],
            evidence: vec![evidence("weak", "q-main", "blue", 1, 10)],
            ground_truth: Some(HypothesisId("h-a".into())),
            expected_recommendation: Recommendation::Recommend {
                query_id: "q-main".into(),
            },
        });
    }
    for index in 0..40 {
        let hypotheses = vec![
            hypothesis("h-a", &[("q-delay", "now"), ("q-check", "one")]),
            hypothesis("h-b", &[("q-delay", "later"), ("q-check", "two")]),
            hypothesis("h-c", &[("q-delay", "never"), ("q-check", "two")]),
        ];
        cases.push(EpistemicInvestigation {
            id: format!("epistemic-stale-{index:03}"),
            hypotheses,
            queries: vec![query("q-delay"), query("q-check")],
            evidence: vec![EvidenceRecord {
                valid_until: Some(0),
                ..evidence("stale", "q-delay", "later", 1, 100)
            }],
            ground_truth: Some(HypothesisId("h-a".into())),
            expected_recommendation: Recommendation::Recommend {
                query_id: "q-delay".into(),
            },
        });
    }
    for index in 0..30 {
        let hypotheses = vec![
            hypothesis("h-a", &[("q-tie", "x"), ("q-tie-2", "left")]),
            hypothesis("h-b", &[("q-tie", "y"), ("q-tie-2", "right")]),
            hypothesis("h-c", &[("q-tie", "x"), ("q-tie-2", "right")]),
        ];
        cases.push(EpistemicInvestigation {
            id: format!("epistemic-unresolved-{index:03}"),
            hypotheses,
            queries: vec![query("q-tie"), query("q-tie-2")],
            evidence: vec![],
            ground_truth: None,
            expected_recommendation: Recommendation::Ambiguous {
                query_ids: vec!["q-tie".into(), "q-tie-2".into()],
            },
        });
    }
    cases
