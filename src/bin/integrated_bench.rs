...
fn main() {
    ...
    let composition_path = env::args()
        .nth(2)
        .unwrap_or_else(|| "data/cross_vertical_ood_v1.json".into());
    let planner_path = env::args()
        .nth(3)
        .unwrap_or_else(|| "data/compositional_planner_ood_v1.json".into());
    let raw_path = env::args()
        .nth(4)
        .unwrap_or_else(|| "data/raw_decomposition_ood_v1.json".into());
    let raw_v2_path = env::args()
        .nth(5)
        .unwrap_or_else(|| "data/raw_decomposition_ood_v2.json".into());
    let mixed: MixedOodCorpus =
        serde_json::from_str(&fs::read_to_string(&mixed_path).expect("mixed corpus"))
            .expect("mixed JSON");
    let composition: CompositionCorpus =
        serde_json::from_str(&fs::read_to_string(&composition_path).expect("composition corpus"))
            .expect("composition JSON");
    let planner: PlannerCorpus =
        serde_json::from_str(&fs::read_to_string(&planner_path).expect("planner corpus"))
            .expect("planner JSON");
    let raw: RawCorpus =
        serde_json::from_str(&fs::read_to_string(&raw_path).expect("raw decomposition corpus"))
            .expect("raw decomposition JSON");
    let raw_v2: RawCorpus = serde_json::from_str(
        &fs::read_to_string(&raw_v2_path).expect("raw decomposition v2 corpus"),
    )
    ...
    assert_eq!(
        composition_report.metrics.final_replay_verified,
        composition_report.metrics.authorized
    );
    ...
    assert_eq!(
        raw_report.metrics.structural_correct,
        raw_report.corpus_cases
    );
    assert_eq!(raw_v2_report.metrics.false_authorizations, 0);
    assert_eq!(raw_v2_report.metrics.false_denials, 0);
    assert_eq!(
        raw_v2_report.metrics.structural_correct,
        raw_v2_report.corpus_cases
    );
    ...
    println!(
        "composition_replay: intermediate={} final={} forged_rejected={} incompatible_rejected={}",
        composition_report.metrics.intermediate_replay_verified,
        composition_report.metrics.final_replay_verified,
        composition_report.metrics.forged_intermediates_rejected,
        composition_report.metrics.incompatible_handoffs_rejected
    );
    println!(
        "planner: replayed_stages={} ambiguous={} invalid_handoffs={}",
        planner_report.metrics.accepted_replayed_stages,
        planner_report.metrics.ambiguous,
        planner_report.metrics.invalid_handoffs_rejected
    );
    println!(
        "raw_decomposition: structural={} ambiguous={} replayed_stages={}",
        raw_report.metrics.structural_correct,
        raw_report.metrics.ambiguous_preserved,
        raw_report.metrics.replayed_stages
    );
    println!(
        "raw_decomposition_v2: structural={} ambiguous={} replayed_stages={}",
        raw_v2_report.metrics.structural_correct,
        raw_v2_report.metrics.ambiguous_preserved,
        raw_v2_report.metrics.replayed_stages
    );
}
