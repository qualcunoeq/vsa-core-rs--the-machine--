...
fn main() {
    let path = env::args().nth(1).unwrap_or_else(|| "data/raw_decomposition_ood_v1.json".into());
    let corpus: RawCorpus = serde_json::from_str(&fs::read_to_string(&path).expect("raw corpus")).expect("raw JSON");
    assert!(corpus.cases.len() == 500);
