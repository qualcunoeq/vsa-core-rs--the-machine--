...
use the_machine::linear_algebra_pack::{
    evaluate_linear_algebra, LinearAlgebraArtifact, LinearAlgebraStatus,
};
...
fn algebra_number(index: usize, expected: Expected) -> Audit {
    ...
    let gate = if expected == Expected::Supported {
        "none"
    } else if expected == Expected::Ambiguous {
        "unresolved_coprimality"
    } else {
        "nonunit"
    };
    audit(
        expected == Expected::Supported && handoff,
        handoff,
        replay,
        tamper,
        2,
...
fn count_probability(index: usize, expected: Expected) -> Audit {
    ...
    let mut probability_request = probability_request(
        vec![
            rational(value as i128, denominator as i128),
            rational(5, denominator as i128),
        ],
...
fn graph_linear(index: usize, expected: Expected) -> Audit {
    ...
    audit(
        ...
        handoff,
        replay,
        !graph_copy.replay_verified() && linear_tamper,
        2,
...
fn spectral_polynomial_number(index: usize, expected: Expected) -> Audit {
    ...
    let mut spectral_request = SpectralRequest {
        ...
        ambiguity: (expected == Expected::Ambiguous)
            .then(|| "spectral operation is unresolved".into()),
    ...
    audit(
        ...
        tamper,
        3,
        usize::from(spectral.artifact.is_some())
            + usize::from(polynomial_replay)
            + usize::from(number_replay),
        gate,
    )
}
...
fn ode_calculus(index: usize, expected: Expected) -> Audit {
    ...
    if expected == Expected::Ambiguous {
        ode_request.ambiguity =
            Some("sampled difference versus continuous derivative is unresolved".into());
    }
    if expected == Expected::Refused {
    ...
    let expression = format!(
        "{}+{}*x",
        ode_request.initial.as_ref().unwrap().numerator,
        ode_request.forcing.as_ref().unwrap().numerator
    );
    let calculus = evaluate_calculus(&CalculusRequest {
        ...
        point: None,
        ambiguity: (expected == Expected::Ambiguous)
            .then(|| "continuous interpretation is unresolved".into()),
        provenance: vec!["stage89-ode-calculus-handoff".into()],
    });
    let handoff = ode.status == OdeStatus::Complete
        && calculus.status == CalculusStatus::Complete
        && calculus.artifact.is_some();
    ...
    audit(
        ...
        handoff,
        replay,
        !ode_copy.replay_verified() && !calculus_copy.replay_verified(),
        2,
        2,
        gate,
    )
...
fn probability_markov(index: usize, expected: Expected) -> Audit {
    let initial = if index % 2 == 0 {
        vec![rational(3, 4), rational(1, 4)]
    } else {
        vec![rational(2, 3), rational(1, 3)]
    };
    let mut probability_request =
        probability_request(initial.clone(), "stage89-probability-markov-handoff");
    if expected == Expected::Ambiguous {
        probability_request.ambiguity = Some("row/column convention is unresolved".into());
    }
    let probability = evaluate_probability(&probability_request);
    ...
    audit(
        ...
        handoff,
        replay,
        !probability_copy.replay_verified() && !markov_copy.replay_verified(),
        2,
        2,
        gate,
    )
...
fn source_sequence(
    ...
    let execution = frontend
        .request
        .as_ref()
        .map(|request| evaluate_formula_records(request, SEQUENCE_DOMAIN, records));
    let handoff = frontend.status == SequenceFrontendStatus::Complete
        && execution.as_ref().is_some_and(|result| {
            result.status == FormulaStatus::Complete && result.value.is_some()
        });
    let replay = sequence_replay(&frontend)
        && execution
            .as_ref()
            .is_none_or(|result| result.replay_verified());
    let mut front_copy = frontend.clone();
    front_copy.replay_hash.push('x');
    let mut tamper = !sequence_replay(&front_copy);
    if let Some(result) = execution.as_ref() {
        let mut copy = result.clone();
        copy.replay_hash.push('x');
        tamper &= !copy.replay_verified();
    }
    let gate = if expected == Expected::Supported {
        "none"
    } else if expected == Expected::Ambiguous {
    ...
    audit(
        ...
        tamper,
        2,
        usize::from(frontend.request.is_some()) + usize::from(execution.is_some()),
        gate,
    )
}
...
fn source_unit(
    ...
    let text = match expected {
        Expected::Supported => format!(
            "Convert {}.5 meters to centimeters using the catalog relation.",
            2 + index % 9
        ),
    ...
    let execution = frontend
        .request
        .as_ref()
        .map(|request| evaluate_formula_records(request, UNIT_DOMAIN, records));
    let handoff = frontend.status == UnitFrontendStatus::Complete
        && execution.as_ref().is_some_and(|result| {
            result.status == FormulaStatus::Complete && result.value.is_some()
        });
    let replay = unit_replay(&frontend)
        && execution
            .as_ref()
            .is_none_or(|result| result.replay_verified());
    let mut front_copy = frontend.clone();
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    std::fs::write(
        "docs/stage89_full_curriculum_synthesis.json",
        format!("{serialized}\n"),
    )?;
    println!("{serialized}");
    Ok(())
