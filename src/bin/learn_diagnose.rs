...
const PHASE1_EPISODES: &[Episode] = &[
    Episode {
        error_text: "bind() to 0.0.0.0:80 failed (98: Unknown error)",
        expected_category: "port_conflict",
        is_novel: false,
    },
    Episode {
    ...
    Episode {
        error_text: "socket.error: [Errno 98] EADDRINUSE",
        expected_category: "port_conflict",
        is_novel: false,
    },
    Episode {
    ...
    Episode {
        ...
        expected_category: "connection_refused",
        is_novel: false,
    },
];
...
fn main() {
    ...
    for (i, ep) in PHASE1_EPISODES.iter().enumerate() {
        eprintln!(
            "  Episode {}: \"{}…\"",
            i + 1,
            &ep.error_text[..ep.error_text.len().min(40)]
        );
    ...
    eprintln!();
    eprintln!(
        "── Phase 2: Testing {} novel variants ───────────",
        PHASE2_EPISODES.len()
    );
    eprintln!();
    ...
    for (i, ep) in PHASE2_EPISODES.iter().enumerate() {
        ...
        match (category_from_classifier, category_from_brain) {
            ...
            {
                eprintln!(
                    "    → Classified by classifier (Level {}) as: {}",
                    level, cat
                );
                classified += 1;
            }
            (None, Some((cat, _conf))) => {
                eprintln!(
                    "    → Classified by brain centroids as: {} (conf={:.4})",
                    cat, _conf
                );
                classified += 1;
            }
    ...
    eprintln!(
        ...
        PHASE1_EPISODES.len() + PHASE2_EPISODES.len()
    );
    ...
    eprintln!(
        "  Fresh classifier (no Phase 1): {}",
        if before_svo.is_some() {
            "trigger match (hardcoded)"
        } else {
            "no match"
        }
    );
    eprintln!(
        "  Learned classifier (Phase 1 absorbed): Level {} → {}",
        after_level,
    ...
    for (text, _expected_category) in ZERO_OVERLAP_TESTS {
        ...
        if from_classifier {
            eprintln!(
                "    Level {}/1-2: {} (UNEXPECTED — should need structural)",
                level,
                svo.unwrap().2
            );
        } else {
        ...
        match structural_triples {
            Some(triples) => {
                eprintln!(
                    "    Level 3 structural: {} triples generated",
                    triples.len()
                );
                ...
                let has_storage_full = triples.contains(&(
                    "storage".to_string(),
                    "has_state".to_string(),
                    "capacity_exhausted".to_string(),
                ));

                if has_network_service || has_unavailable {
                    eprintln!(
                        "    → Bridge to: network_service_unavailable (port conflict / connection)"
                    );
    ...
    eprintln!(
        "  Zero-overlap tests: {} / {} structurally classified",
