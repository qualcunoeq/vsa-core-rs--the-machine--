...
use crate::epistemic::{
    analyze, EpistemicAnalysis, EpistemicInvestigation, EvidenceRecord, Hypothesis, HypothesisId,
    InformationGain, Recommendation, MIN_DECISIVE_EVIDENCE_SCORE,
};
...
fn open_set_hash(
    ...
    hasher.update(
        serde_json::to_vec(&(outcome, unexplained, proposal_id))
            .expect("open-set receipt serializes"),
    );
...
fn information_gain_for_query(
    ...
    for hypothesis in hypotheses {
        let outcome = hypothesis
            .predictions
            .get(query_id)
            .cloned()
            .unwrap_or_else(|| "unmodeled".into());
...
fn proposal_for(
    unexplained: &[&EvidenceRecord],
    investigation: &EpistemicInvestigation,
) -> Option<HypothesisProposal> {
    let record = unexplained.first()?;
    let mut predictions = BTreeMap::new();
    predictions.insert(record.query_id.clone(), record.outcome.clone());
    let overlap_hypotheses = investigation
        .hypotheses
        .iter()
        .filter(|hypothesis| hypothesis.predictions.contains_key(&record.query_id))
        .map(|hypothesis| hypothesis.id.clone())
        .collect::<Vec<_>>();
    let expected_information_gain =
        information_gain_for_query(&investigation.hypotheses, &record.query_id, &record.outcome);
    ...
    Some(HypothesisProposal {
        ...
        minimum_latent_cause: format!(
            "an unmodeled cause producing {} for {}",
            record.outcome, record.query_id
        ),
        predictions,
        overlap_hypotheses,
        assumptions: vec![format!("the {} observation is reliable", record.id)],
        falsification_conditions: vec![format!(
            "reliable evidence on {} matches the active hypotheses instead",
            record.query_id
        )],
...
pub fn analyze_open_set(investigation: &OpenSetInvestigation) -> OpenSetAnalysis {
    ...
    let unexplained = reliable
        .iter()
        .filter(|record| {
            investigation.epistemic.hypotheses.iter().all(|hypothesis| {
                hypothesis.predictions.get(&record.query_id) != Some(&record.outcome)
            })
        })
        .copied()
        .collect::<Vec<_>>();
    ...
    let outcome = if reliable.is_empty() {
        ...
        if proposal_for(&unexplained, &investigation.epistemic).is_some() {
            OpenSetOutcome::NovelHypothesisNeeded
        } else {
            OpenSetOutcome::NoAdequateHypothesis
        }
    } else if epistemic.plausible_hypotheses.len() == 1 {
        OpenSetOutcome::BestKnownHypothesis {
            hypothesis_id: epistemic.plausible_hypotheses[0].clone(),
        }
    } else if epistemic.plausible_hypotheses.len() > 1 {
        OpenSetOutcome::MultiplePlausibleHypotheses {
            hypothesis_ids: epistemic.plausible_hypotheses.clone(),
        }
    ...
    OpenSetAnalysis {
        epistemic,
        outcome,
        unexplained_observations: unexplained.iter().map(|record| record.id.clone()).collect(),
        proposal: proposal_for(&unexplained, &investigation.epistemic),
    }
}
...
pub fn replay_open_set(investigation: &OpenSetInvestigation) -> OpenSetReplayReceipt {
    ...
    let proposal_id = analysis
        .proposal
        .as_ref()
        .map(|proposal| proposal.proposal_id.clone());
    let replay_hash = open_set_hash(
        &analysis.outcome,
        &analysis.unexplained_observations,
        &proposal_id,
    );
    OpenSetReplayReceipt {
...
pub fn evaluate_corpus(cases: &[OpenSetInvestigation]) -> OpenSetBenchmarkReport {
    let mut report = OpenSetBenchmarkReport {
        cases: cases.len(),
        ..Default::default()
    };
    for case in cases {
        ...
        if matches!(case.expected_outcome, OpenSetOutcome::NovelHypothesisNeeded) {
            ...
            report.missing_hypothesis_detected += usize::from(matches!(
                analysis.outcome,
                OpenSetOutcome::NovelHypothesisNeeded
            ));
            report.proposal_cases += 1;
            report.proposals_falsifiable += usize::from(
                analysis
                    .proposal
                    .as_ref()
                    .is_some_and(|proposal| !proposal.falsification_conditions.is_empty()),
            );
        }
        ...
        if case.true_hypothesis.as_ref().is_some_and(|truth| {
            case.epistemic
                .hypotheses
                .iter()
                .any(|hypothesis| &hypothesis.id == truth)
        }) {
            report.calibration_cases += 1;
            report.calibration_retained += usize::from(
                analysis
                    .epistemic
                    .plausible_hypotheses
                    .contains(case.true_hypothesis.as_ref().expect("known truth")),
            );
        }
        report.recommendation_quality += usize::from(
            analysis
                .proposal
                .as_ref()
                .is_none_or(|proposal| proposal.expected_information_gain.numerator > 0),
        );
...
fn hypothesis(id: &str, query: &str, outcome: &str) -> Hypothesis {
    Hypothesis {
        ...
        predictions: [(query.into(), outcome.into())].into_iter().collect(),
...
pub fn synthetic_corpus() -> Vec<OpenSetInvestigation> {
    ...
    for index in 0..80 {
        ...
        let base = EpistemicInvestigation {
            id: format!("open-included-{index:03}"),
            hypotheses,
            queries: vec![query("q")],
            evidence: vec![evidence("included", "q", "a", 100, None)],
            ground_truth: Some(HypothesisId("h-a".into())),
            expected_recommendation: Recommendation::NoDiscriminatingEvidence,
        };
        cases.push(OpenSetInvestigation {
            id: base.id.clone(),
            epistemic: base,
            expected_outcome: OpenSetOutcome::BestKnownHypothesis {
                hypothesis_id: HypothesisId("h-a".into()),
            },
            true_hypothesis: Some(HypothesisId("h-a".into())),
            introduced_hypothesis: None,
        });
    }
    for index in 0..80 {
        let hypotheses = vec![hypothesis("h-a", "q", "a"), hypothesis("h-b", "q", "b")];
        let base = EpistemicInvestigation {
            id: format!("open-omitted-{index:03}"),
            hypotheses,
            queries: vec![query("q")],
            evidence: vec![evidence("omitted", "q", "c", 100, None)],
            ground_truth: Some(HypothesisId("h-c".into())),
            expected_recommendation: Recommendation::NoDiscriminatingEvidence,
        };
        cases.push(OpenSetInvestigation {
            id: base.id.clone(),
            epistemic: base,
            expected_outcome: OpenSetOutcome::NovelHypothesisNeeded,
            true_hypothesis: Some(HypothesisId("h-c".into())),
            introduced_hypothesis: Some(hypothesis("h-c", "q", "c")),
        });
    }
    for index in 0..40 {
        let hypotheses = vec![hypothesis("h-a", "q", "a"), hypothesis("h-b", "q", "b")];
        let base = EpistemicInvestigation {
            id: format!("open-inadequate-{index:03}"),
            hypotheses,
            queries: vec![query("q")],
            evidence: vec![evidence("anomaly", "q", "unknown", 100, None)],
            ground_truth: None,
            expected_recommendation: Recommendation::NoDiscriminatingEvidence,
        };
        cases.push(OpenSetInvestigation {
            ...
            introduced_hypothesis: Some(hypothesis("h-new", "q", "unknown")),
        });
    }
    for index in 0..30 {
        let hypotheses = vec![hypothesis("h-a", "q", "a"), hypothesis("h-b", "q", "b")];
        let base = EpistemicInvestigation {
            id: format!("open-misleading-{index:03}"),
            hypotheses,
            queries: vec![query("q")],
            evidence: vec![evidence("misleading", "q", "a", 100, None)],
            ground_truth: Some(HypothesisId("h-b".into())),
            expected_recommendation: Recommendation::NoDiscriminatingEvidence,
        };
        cases.push(OpenSetInvestigation {
            id: base.id.clone(),
            ...
            true_hypothesis: Some(HypothesisId("h-b".into())),
            introduced_hypothesis: None,
        });
    }
    for index in 0..20 {
        let hypotheses = vec![hypothesis("h-a", "q", "a"), hypothesis("h-b", "q", "b")];
        let base = EpistemicInvestigation {
            id: format!("open-correlated-{index:03}"),
            hypotheses,
            queries: vec![query("q")],
            evidence: vec![
                evidence("sensor-a", "q", "a", 90, None),
                evidence("sensor-b", "q", "a", 90, None),
            ],
            ground_truth: Some(HypothesisId("h-a".into())),
            expected_recommendation: Recommendation::NoDiscriminatingEvidence,
        };
    ...
    for index in 0..20 {
        let hypotheses = vec![hypothesis("h-a", "q", "a"), hypothesis("h-b", "q", "b")];
        let base = EpistemicInvestigation {
            id: format!("open-stale-{index:03}"),
            hypotheses,
            queries: vec![query("q")],
            evidence: vec![evidence("stale", "q", "a", 100, Some(0))],
            ground_truth: Some(HypothesisId("h-a".into())),
            expected_recommendation: Recommendation::NoDiscriminatingEvidence,
        };
        cases.push(OpenSetInvestigation {
            id: base.id.clone(),
            epistemic: base,
            expected_outcome: OpenSetOutcome::InsufficientEvidence,
            true_hypothesis: Some(HypothesisId("h-a".into())),
            introduced_hypothesis: None,
        });
    }
    for index in 0..20 {
        let hypotheses = vec![hypothesis("h-a", "q", "a"), hypothesis("h-b", "q", "b")];
        let base = EpistemicInvestigation {
            id: format!("open-adversarial-{index:03}"),
            hypotheses,
            queries: vec![query("q")],
            evidence: vec![evidence("adversarial", "q", "unknown", 100, None)],
            ground_truth: None,
            expected_recommendation: Recommendation::NoDiscriminatingEvidence,
        };
        cases.push(OpenSetInvestigation {
    ...
    for index in 0..10 {
        let hypotheses = vec![
            hypothesis("h-a", "q", "same"),
            hypothesis("h-b", "q", "same"),
        ];
        let base = EpistemicInvestigation {
            id: format!("open-no-preference-{index:03}"),
            hypotheses,
            queries: vec![query("q")],
            evidence: vec![evidence("shared", "q", "same", 100, None)],
            ground_truth: None,
            expected_recommendation: Recommendation::NoDiscriminatingEvidence,
        };
        cases.push(OpenSetInvestigation {
            id: base.id.clone(),
            epistemic: base,
            expected_outcome: OpenSetOutcome::MultiplePlausibleHypotheses {
...
mod tests {
    ...
    fn open_set_detects_missing_hypotheses_without_promotion() {
        ...
        assert!(matches!(
            analyze_open_set(&cases[80]).outcome,
            OpenSetOutcome::NovelHypothesisNeeded
        ));
